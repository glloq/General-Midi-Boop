...
Everything else that ships with a build has its own terms:

- **npm dependencies** (271 runtime packages, all permissive) —
  [`THIRD-PARTY-NOTICES.md`](./THIRD-PARTY-NOTICES.md), regenerated by
  `node scripts/audit/licenses.mjs --emit`.
- **Icons, soundfont, vendored browser libraries** —
  [`assets/ASSET-LICENSES.md`](./assets/ASSET-LICENSES.md).

> **Before you redistribute a build, read `assets/ASSET-LICENSES.md`.** Two items
> are unresolved as of 2026-09-08: `public/lib/WebAudioFontPlayer.js` is fetched at
> install time from an upstream that is now **GPL-3.0-or-later**, and 57 of the
> shipped icons came through SVG Repo with no traceable licence. Neither is covered
