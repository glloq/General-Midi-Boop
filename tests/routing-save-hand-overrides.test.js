...
describe('routing_save_hand_overrides — input validation', () => {
  test('throws when fileId is missing', async () => {
    await expect(
      routingSaveHandOverrides(makeApp(), { channel: 0, deviceId: 'p' })
    ).rejects.toThrow(ValidationError);
  });

  test('throws when deviceId is missing', async () => {
    await expect(routingSaveHandOverrides(makeApp(), { fileId: 1, channel: 0 })).rejects.toThrow(
      ValidationError
    );
  });

  test('throws when channel is missing', async () => {
    await expect(routingSaveHandOverrides(makeApp(), { fileId: 1, deviceId: 'p' })).rejects.toThrow(
      ValidationError
    );
  });
  ...
  test('throws when channel is out of range', async () => {
    await expect(
      routingSaveHandOverrides(makeApp(), {
        fileId: 1,
        deviceId: 'p',
        channel: 99
      })
  ...
  test('throws when a hand_anchor entry is malformed', async () => {
    await expect(
      routingSaveHandOverrides(makeApp(), {
        ...
        channel: 0,
        overrides: {
          hand_anchors: [{ tick: 0, handId: 'left' /* anchor missing */ }],
          disabled_notes: []
        }
      })
...
describe('routing_save_hand_overrides — happy paths', () => {
  test('forwards a valid payload to the repo and returns updated count', async () => {
    ...
    expect(app.routingRepository.saveHandOverrides).toHaveBeenCalledWith(
      42,
      3,
      'piano-1',
      validOverrides
