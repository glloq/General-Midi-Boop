...
describe('validateBagpipeConfigPayload', () => {
  ...
  test('well-formed config accepted', () => {
    expect(() =>
      validateBagpipeConfigPayload({ drones: [45, 45, 33], enabled: true })
    ).not.toThrow();
  ...
  test('malformed object-shaped drones rejected', () => {
    expect(() => validateBagpipeConfigPayload({ drones: [{ note: 128, enabled: true }] })).toThrow(
      ValidationError
    );
    expect(() => validateBagpipeConfigPayload({ drones: [{ note: 45, enabled: 'x' }] })).toThrow(
      ValidationError
    );
    expect(() =>
      validateBagpipeConfigPayload({ drones: [45, { note: 33, enabled: true }] })
...
describe('validateAccordionConfigPayload', () => {
  ...
  test('invalid enum values rejected', () => {
    ...
    expect(() => validateAccordionConfigPayload({ right_display: 'touch' })).toThrow(
      ValidationError
    );
  });
  ...
  test('well-formed bass_range accepted (incl. single bound / empty)', () => {
    expect(() =>
      validateAccordionConfigPayload({ bass_range: { min: 36, max: 60 } })
    ).not.toThrow();
  ...
  test('malformed / inverted bass_range rejected', () => {
    expect(() => validateAccordionConfigPayload({ bass_range: { min: 70, max: 40 } })).toThrow(
      ValidationError
    );
    expect(() => validateAccordionConfigPayload({ bass_range: { min: -1 } })).toThrow(
      ValidationError
    );
    expect(() => validateAccordionConfigPayload({ bass_range: { max: 200 } })).toThrow(
      ValidationError
    );
    expect(() => validateAccordionConfigPayload({ bass_range: { min: 1.5 } })).toThrow(
      ValidationError
    );
  ...
  test('malformed Stradella geometry rejected', () => {
    ...
    expect(() => validateAccordionConfigPayload({ bass_funcs: ['bogus'] })).toThrow(
      ValidationError
    );
  });
});
