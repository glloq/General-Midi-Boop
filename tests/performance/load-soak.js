...
const silentLogger = {
  ...
  isDebugEnabled: () => false, isWarnEnabled: () => true, isInfoEnabled: () => true
};
...
function record(name, value, unit, budget, ok) {
  ...
  console.log(
    `  [${tag}] ${name}: ${value}${unit}` + (budget ? `  (budget ${budget})` : '')
  );
}
...
function makeState() {
  return {
    playing: true, paused: false, position: 0, duration: 1e9,
    events: [], currentEventIndex: 0, startTime: 0, playbackRate: 1,
    loop: false, channelRouting: new Map(), mutedChannels: new Set(),
...
function buildEvents(n) {
  ...
  for (let i = 0; i < n; i++) {
    const ch = i & 15;
    const m = i % 3;
    if (m === 0) evs[i] = { time: 0, type: 'noteon', channel: ch, note: 60 + (i % 24), velocity: 96 };
    else if (m === 1) evs[i] = { time: 0, type: 'noteoff', channel: ch, note: 60 + (i % 24), velocity: 0 };
...
async function scenarioLargeMidi() {
  ...
  eld.disable();
  try { scheduler.destroy(); } catch { /* ignore */ }
...
function scenarioWsFlood() {
  ...
  const q = new WsOutputQueue({
    ...
    logger: silentLogger,
    scheduleFlush: (cb) => { pendingFlush = cb; }
  });
  ...
  record('B.max_queue_depth', maxDepth, '', `≤ ${DEFAULT_MAX_QUEUE_DEPTH}`, maxDepth <= DEFAULT_MAX_QUEUE_DEPTH);
...
(async () => {
  ...
  if (failed.length) {
    for (const f of failed) console.log(`   breached: ${f.name} = ${f.value}${f.unit} (budget ${f.budget})`);
    process.exit(1);
  }
})().catch((e) => { console.error('load-soak harness error:', e); process.exit(2); });
