...
beforeAll(() => {
  const src = readFileSync(resolve(__dirname, '../../public/js/utils/MidiConstants.js'), 'utf8');
  new Function(src)();
});
...
describe('MidiConstants.normalizeBagpipeDrones', () => {
  ...
  it('object shape preserves enabled and drops out-of-range', () => {
    expect(norm([{ note: 45, enabled: false }, { note: 200 }])).toEqual([
      { note: 45, enabled: false }
    ]);
  });

  it('rounds non-integers and treats missing enabled as true', () => {
    expect(norm([45.4, { note: 33.6 }])).toEqual([
      { note: 45, enabled: true },
      { note: 34, enabled: true }
    ]);
  });
