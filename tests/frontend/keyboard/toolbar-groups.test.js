...
const ALL_KINDS = [
  'piano', 'piano-slider', 'fretboard', 'drumpad', 'keyboard-list',
  'harmonica', 'harp', 'accordion', 'mallet', 'kalimba', 'bagpipe',
  'steel-drum', 'theremin'
];
...
describe('PE-2 — _applyToolbarGroups behaviour-faithful extraction', () => {
  ...
  it('toggling across modes flips visibility back and forth', () => {
    ...
    m.viewMode = 'piano';      m._applyToolbarGroups(); expect(ob()).toBe(false);
    m.viewMode = 'drumpad';    m._applyToolbarGroups(); expect(ob()).toBe(true);
    m.viewMode = 'piano-slider'; m._applyToolbarGroups(); expect(ob()).toBe(false);
  });
});
...
describe('PE-2 — scope isolation (caps-aware / view-mode groups untouched)', () => {
  ...
  const FOREIGN = [
    'keyboard-view-mode-group', 'velocity-control-panel',
    'modulation-control-panel', 'pitch-bend-control-panel',
    'keyboard-piano-slider-group', 'keyboard-list-cc-group',
    'keyboard-list-pb-group', 'wind-instrument-panel'
  ];
  ...
  it('leaves every out-of-scope group exactly as it was', () => {
    ...
    for (const vm of ALL_KINDS) { m.viewMode = vm; m._applyToolbarGroups(); }
