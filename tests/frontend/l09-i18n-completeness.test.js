...
describe('L09 · i18n — reference corpus', () => {
  ...
  // 58 clés de la modal Audio → MIDI (`transcription.*` +
  // `settings.transcriptionButton.title`), traduites dans les 28 locales.
  it('measures 28 locales against a 2 862-key English reference', () => {
    expect(locales).toHaveLength(28);
    expect(keys.length).toBe(2862);
  });
});
