  ...
  const ctx = new Proxy(
    { calls },
    {
      get(target, prop) {
        ...
        if (
          typeof prop === 'string' &&
          /^(setTransform|fillRect|strokeRect|fillText|beginPath|moveTo|lineTo|closePath|fill|stroke|clearRect|save|restore|translate|scale|rotate|setLineDash|rect|clip|arc|bezierCurveTo|quadraticCurveTo)$/.test(
            prop
          )
        ) {
...
describe('HandsLookaheadStrip — _firstVisibleIndex', () => {
  it('returns 0 when sec is before all notes', () => {
    const s = makeStrip({
      notes: [
        { tick: 480, note: 60, duration: 240 },
        { tick: 1920, note: 64, duration: 240 }
      ]
    });
...
describe('HandsLookaheadStrip — drawing (vertical fall-down)', () => {
  ...
  it('honours setCurrentTime — notes that scrolled past disappear', () => {
    ...
    const s = new window.HandsLookaheadStrip(makeCanvas(), {
      ...
      notes: [
        { tick: 0, note: 60, duration: 240 },
        { tick: 4800, note: 64, duration: 240 }
      ]
    });
  ...
  it('infeasible transition extends past the chord tick by (requiredSec − availableSec)', () => {
    ...
    function transitionTopY(calls, rePattern) {
      ...
      for (const c of calls) {
        ...
        if (!inPath) continue;
        if (c.method === 'fill' || c.method === 'closePath') {
          inPath = false;
          continue;
        }
