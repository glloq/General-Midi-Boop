...
describe('L11 §AG — F-14 : le repli CDN bloquant de public/index.html (CORRIGÉ, R6)', () => {
  ...
  test("les 196 balises <script src> sont désormais toutes locales : plus rien n'est derrière un appel réseau", () => {
    const srcs = [...indexHtml.matchAll(/<script src="([^"]+)"/g)].map((m) => m[1]);
    // 191 à R6 ; +2 en vague 3 / R12 (modale de routage live + son lanceur) ;
    // +1 en vague 4 / R20 (features/transport/PlaybackResync.js) ; +1 pour la
    // modale Audio → MIDI (features/transcription/TranscriptionModal.js) ;
    // +1 pour sa section de réglages (features/settings/SettingsTranscription.js),
    // toutes en chemin relatif. Le compte reste exact pour qu'ajouter un
    // script soit une modification consciente de ce test ; l'invariant protégé
    // est l'assertion ci-dessous : aucune balise ne vise une origine distante.
    expect(srcs.length).toBe(196);
