...
(function() {
    ...
    function showToast(message, type = 'info', opts = {}) {
        ...
        el.style.cssText = baseStyle(24).join(';')
    ...
    function withProgressToast(promiseOrFn, message, opts = {}) {
        ...
        let handle = null;
        const timer = setTimeout(() => { handle = showProgressToast(message); }, threshold);
        const cleanup = () => { clearTimeout(timer); if (handle) handle.done(); };
        return p.then(
            (v) => { cleanup(); return v; },
            (e) => { cleanup(); throw e; }
        );
