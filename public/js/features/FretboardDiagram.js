...
class FretboardDiagram {
  ...
  _onMouseDown(e) {
    ...
    if (midiNote >= 0 && midiNote <= 127) {
      this.canvas.dispatchEvent(
        new CustomEvent('fretboard:click', {
      ...
      const releaseHandler = () => {
        canvas.dispatchEvent(
          new CustomEvent('fretboard:release', {
            detail: { string: closestString, fret: clickedFret, midiNote },
            bubbles: false
          })
        );
  ...
  /** Set of string numbers that currently have an active position. */
  _activeStringSet() {
    return new Set(this.activePositions.map((p) => p.string));
  }
  ...
  _drawStrings(w, h) {
    ...
    const activeStringSet = this._activeStringSet();
