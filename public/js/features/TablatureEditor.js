...
class TablatureEditor {
  ...
  async show(stringInstrument, midiNotes, channel) {
    ...
    if (this.modal.editActions.updateUndoRedoButtonsState)
      this.modal.editActions.updateUndoRedoButtonsState();
  ...
  _updateLabels() {
    ...
    if (badgeEl) {
      ...
      const nStrings =
        this.stringInstrument.tuning?.length || this.stringInstrument.num_strings || 6;
    ...
    if (tuningEl && this.stringInstrument.tuning) {
      ...
      tuningEl.textContent = this.stringInstrument.tuning.map((n) => noteNames[n % 12]).join('-');
    }
  ...
  _initRenderer() {
    ...
    if (this.modal.pianoRoll) {
      ...
      if (pr.xrange && this.tabCanvasEl.width) {
        this.renderer.ticksPerPixel =
          pr.xrange / (this.tabCanvasEl.width - this.renderer.headerWidth);
      }
      if (pr.xoffset) {
  ...
  async convertFromMidi(midiNotes) {
    ...
    try {
      ...
      if (response && response.tablature) {
        ...
        const dropped =
          response.stats && typeof response.stats.dropped === 'number'
  ...
  _updateFretboardHandWindow(tick) {
    ...
    const span =
      Number.isFinite(this._handSpanFrets) && this._handSpanFrets > 0 ? this._handSpanFrets : 4;
