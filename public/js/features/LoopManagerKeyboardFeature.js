  ...
  class LoopManagerKeyboardFeature {
            ...
            if (this.synth) {
              try {
                if (this.isDrum) {
                  ...
                  this.synth
                    .loadDrumKit?.()
                    .catch((err) => LoopUtils.handleError(err, 'kbd.synth.loadDrumKit'));
                } else {
                  this.synth.setChannelInstrument(0, this.instrument);
                  if (!this.synth.loadedInstruments?.has(this.instrument)) {
                    this.synth
                      .loadInstrument(this.instrument)
                      .catch((err) => LoopUtils.handleError(err, 'kbd.synth.loadInstrument'));
                  }
                }
              } catch (err) {
                LoopUtils.handleError(err, 'kbd.synth.setChannelInstrument');
              }
            }
    ...
    unmount() {
      try {
        ...
        LoopUtils.handleError(err, 'kbd.unmount');
      }
      this.mounted = false;
      this.stopAllNotes();
    }
    ...
    noteOn(note, velocity = 80) {
      ...
      if (
        ch === 9 &&
        typeof this.synth.ensureDrumKitReady === 'function' &&
    ...
    noteOff(note) {
      ...
      for (const e of env) {
        try {
          ...
          LoopUtils.handleError(err, 'kbd.synth.cancel');
        }
      }
