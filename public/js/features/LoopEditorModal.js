...
class LoopEditorModal extends BaseModal {
  constructor(api, eventBus, { onSaved } = {}) {
    ...
    // The user must pick an instrument from the header dropdown before
    // recording — otherwise notes would silently route to the default
    // piano. Set to true once a selection happens, or when an existing
    // loop is loaded (the saved program counts as a valid selection).
    this._instrumentSelected = false;
  }
  ...
  async _loadLoopStateById(id) {
    try {
      ...
      this.sequence = LoopUtils.parseSequence(loop.midi_data).map((n) => ({
        ...
        n: n.n,
        v: n.v ?? 80,
        g: Number.isInteger(n.g) && n.g > 0 ? n.g : Number.isInteger(n.l) && n.l > 0 ? n.l : 120
      }));
      // Loop with a saved program counts as an explicit choice.
  ...
  _renderHeader() {
    ...
    const timeSigHtml = timeSigOptions
      .map(
        ([label, num, den]) =>
  ...
  _initPianoRollEditor() {
    ...
    if (typeof window.MidiEditorModal === 'function') {
      this.midiEditorPanel = new window.MidiEditorModal(this.eventBus, this.api, {
        loopMode: true
      });
      ...
      const ready = this.midiEditorPanel.showAsPanel(host, {
        ...
        onChange: () => {
          this._syncPanelMinimap(); /* dirty tracked via snapshot */
        }
      });
  ...
  _initPanelMinimap() {
    ...
    this._panelMinimapObserver.observe(pr, {
      attributes: true,
      attributeFilter: ['xoffset', 'xrange']
    });
  ...
  _teardownPanelResizeObserver() {
    if (this._panelResizeObs) {
      try {
        this._panelResizeObs.disconnect();
      } catch (_) {
        /* best-effort */
      }
  ...
  _teardownPanelMinimap() {
    if (this._panelMinimapObserver) {
      try {
        this._panelMinimapObserver.disconnect();
      } catch (_) {
        /* best-effort */
      }
    ...
    if (this._panelMinimapCanvasObs) {
      try {
        this._panelMinimapCanvasObs.disconnect();
      } catch (_) {
        /* best-effort */
      }
    ...
    if (this._panelMinimapChange && this.midiEditorPanel?.pianoRoll) {
      try {
        this.midiEditorPanel.pianoRoll.removeEventListener('change', this._panelMinimapChange);
      } catch (_) {
        /* best-effort */
      }
    }
  ...
  onClose() {
    ...
    if (this.midiEditorPanel) {
      try {
        this.midiEditorPanel.unmountPanel();
      } catch (_) {
        /* best-effort */
      }
  ...
  _onClick(e) {
    ...
    if (tabBtn) {
      this._switchTab(tabBtn.dataset.leTab);
      return;
    }
    ...
    switch (btn.dataset.action) {
      ...
      case 'save-loop':
        this._saveLoop();
        break;
      case 'save-loop-as-new':
        this._saveLoop({ asNew: true });
        break;
  ...
  /**
   * Enable / disable the REC button depending on whether the user has
   * picked an instrument. Disabled record button gets a hint title +
   * `aria-disabled` so screen readers don't fire it either.
   */
  _refreshRecButtonEnabled() {
    const btn = this.$('#lc-record-btn');
    if (!btn) return;
    const ok = this._instrumentSelected === true;
    btn.disabled = !ok;
    btn.setAttribute('aria-disabled', ok ? 'false' : 'true');
    btn.title = ok
      ? this.t('loopCreator.record')
      : this.t('loopEditor.chooseInstrumentFirst') ||
        "Choisissez un instrument avant d'enregistrer";
    btn.classList.toggle('le-rec-big--disabled', !ok);
  }
  ...
  _attachKeyboardShortcuts() {
    ...
    this._keyHandler = (e) => {
      ...
      if (e.key === ' ') {
        e.preventDefault();
        this.isPlaying ? this._stopAll() : this._previewLoop();
        return;
      }
      if (e.key === 'Escape') {
        this._stopAll();
      ...
      if (e.key.toLowerCase() === 's' && !mod && !e.shiftKey) {
        ed?.setMode?.('select');
        return;
      }
  ...
  _adjustTempo(d) {
    ...
    const el = this.$('#lc-tempo');
    if (el) el.value = this.tempo;
    this._refreshPianoRollRange();
  }

  _adjustBars(d) {
    this.bars = LoopUtils.validate.editorBars(this.bars + d, this.bars);
    const el = this.$('#lc-bars');
    if (el) el.value = this.bars;
    this._refreshPianoRollRange();
  }
  ...
  _toggleOutput() {
    ...
    this._setStatus(
      this._outputTarget === 'live'
        ? this.t('loopCreator.outputLive')
        : this.t('loopCreator.outputSynth')
    );
  }
  ...
  _previewNoteOn(note, velocity) {
    ...
    if (
      ch === 9 &&
      typeof this._synth.ensureDrumKitReady === 'function' &&
  ...
  _previewNoteOff(note) {
    ...
    for (const env of envelopes) {
      try {
        ...
        LoopUtils.handleError(err, 'editor.live.synth.cancel');
      }
    }
  ...
  _startRecording() {
    // Refuse to record until the user has picked an instrument —
    // otherwise notes go silently to the default piano (channel 0,
    // program 0) and the loop saves with the wrong sound. The
    // user-facing hint nudges them toward the header dropdown.
    if (!this._instrumentSelected) {
      this._setStatus(
        this.t('loopEditor.chooseInstrumentFirst') || "Choisissez un instrument avant d'enregistrer"
      );
      LoopUtils.toast(
        this.t('loopEditor.chooseInstrumentFirst') ||
          "Choisissez un instrument avant d'enregistrer",
      ...
      this.$('#le-instrument-select')?.focus();
      return;
    }
  ...
  _stopRecordingTimer() {
    if (this._recTimerId) {
      clearInterval(this._recTimerId);
  ...
  async _startMidiInMonitor() {
    ...
    try {
      ...
      if (this._monitorSession !== token) {
        ...
        try {
          await this.api.sendCommand('monitor_stop', { deviceId: this._midiInDevice });
        } catch (_) {}
        return;
      }
  ...
  async _stopMidiInMonitor() {
    ...
    if (wasActive && this._midiInDevice) {
      try {
        ...
        LoopUtils.handleError(err, 'editor.midiIn.stop');
      }
    }
  }
  ...
  async _probeKeyboardIdentities() {
    try {
      ...
      const probes = candidates.map((d) => {
        const id = d.device_id || d.id;
        return this.api.sendCommand('sysex_identity_request', { deviceId: id }).catch(() => {}); // input-only / unsupported devices throw — ignore
      });
      if (!probes.length) return;
      await Promise.race([Promise.all(probes), new Promise((res) => setTimeout(res, 150))]);
  ...
  _mountKeyboardPanel() {
    ...
    window.keyboardModal.mountAsPanel(container, {
      ...
      onInstrumentSelected: ({
        ...
        // dropdown so the two stay visually in sync.
        this._instrumentSelected = true;
        const sel = this.$('#le-instrument-select');
        if (sel) {
          const v = String(isDrum ? 128 : (gmProgram ?? 0));
        ...
        try {
          if (isDrum) {
            ...
            this._synth
              ?.loadDrumKit?.()
              .catch((err) => LoopUtils.handleError(err, 'editor.synth.loadDrumKit'));
          } else {
            ...
            if (this._synth && !this._synth.loadedInstruments?.has(this.instrumentProgram)) {
              this._synth
                .loadInstrument(this.instrumentProgram)
                .catch((err) => LoopUtils.handleError(err, 'editor.synth.loadInstrument'));
            }
          }
        } catch (err) {
          LoopUtils.handleError(err, 'editor.synth.setChannelInstrument');
        }
  ...
  _relocateInstrumentSelectorToHeader(kbContainer) {
    ...
    const tryMove = (attempt = 0) => {
      const sel =
        (kbContainer || document).querySelector('#header-instrument-selector') ||
  ...
  _previewLoop() {
    ...
    try {
      if (this._isDrumKit) {
        ...
        this._synth
          .loadDrumKit?.()
          .catch((err) => LoopUtils.handleError(err, 'editor.preview.loadDrumKit'));
      } else {
        this._synth.setChannelInstrument(0, this.instrumentProgram);
      }
    } catch (err) {
      LoopUtils.handleError(err, 'editor.preview.setChannelInstrument');
    }
  ...
  _stopAll() {
    ...
    if (this._synth) {
      ...
      try {
        ...
        LoopUtils.handleError(err, 'editor.synth.stop');
      }
      try {
        this._synth.cancelAllNotes?.();
      } catch (err) {
        LoopUtils.handleError(err, 'editor.synth.cancelAllNotes');
      }
    }
  ...
  async _saveLoop({ asNew = false } = {}) {
    ...
    if (asNew) {
      ...
      const nameEl = this.$('#lc-name-input');
      if (nameEl) nameEl.value = this.loopName;
    }
    ...
    const seq = rawSeq.map((n) => {
      const len =
        Number.isInteger(n.l) && n.l > 0 ? n.l : Number.isInteger(n.g) && n.g > 0 ? n.g : 120;
