...
(function () {
  ...
  LightingHelpersMixin._clamp = function (val, min, max) {
  ...
  LightingHelpersMixin._buildColorPreview = function (action) {
    const pill = (bg) =>
      `<div style="width:28px;height:16px;border-radius:4px;background:${bg};border:1px solid #ddd;flex-shrink:0;"></div>`;
    ...
    if (action.type === 'sparkle') {
      return pill(
        'linear-gradient(135deg,#333 25%,#FFF 30%,#333 35%,#FFF 60%,#333 65%,#FFF 80%,#333 85%)'
      );
    }
  ...
  LightingHelpersMixin._startMidiLearn = async function () {
    ...
    btn.textContent =
      i18n.t('lighting.midiLearnWaiting') || "🎹 En attente d'un événement MIDI... (10s)";
    ...
    try {
      ...
      if (res.success && res.learned) {
        ...
        btn.textContent = (
          i18n.t('lighting.midiLearnCaptured') ||
          '✅ Capturé: {type} ch{channel} note={note} vel={velocity} cc={cc}'
        )
          .replace('{type}', l.type)
          .replace('{channel}', (l.channel || 0) + 1)
          .replace('{note}', l.note ?? '-')
          .replace('{velocity}', l.velocity ?? '-')
          .replace('{cc}', l.controller ?? '-');
        btn.style.borderColor = '#10b981';
        btn.style.color = '#10b981';
      } else {
        btn.textContent =
          i18n.t('lighting.midiLearnNoSignal') || '⏰ Pas de signal MIDI reçu. Réessayez.';
        btn.style.borderColor = '#f59e0b';
        btn.style.color = '#d97706';
      }
    } catch (error) {
      btn.textContent = (i18n.t('lighting.midiLearnError') || '❌ Erreur: {error}').replace(
        '{error}',
        error.message
      );
      btn.style.borderColor = '#ef4444';
    ...
    setTimeout(() => {
      if (btn) {
        btn.textContent =
          i18n.t('lighting.midiLearnDefault') ||
          '🎹 MIDI Learn — Jouez une note pour auto-configurer la condition';
        btn.style.borderColor = '#f59e0b';
        btn.style.color = '#d97706';
        btn.disabled = false;
      }
  ...
  LightingHelpersMixin.showColorWheel = function (targetInputId) {
    ...
    // Sample the canvas at (x,y), update selectedColor + the preview/hex UI.
    const sampleAt = (x, y) => {
    ...
    const pickColor = (e) => {
      ...
      lastPickX = Math.round((e.clientX - rect.left) * scaleX);
      lastPickY = Math.round((e.clientY - rect.top) * scaleY);
      sampleAt(lastPickX, lastPickY);
    };
    ...
    canvas.addEventListener('touchstart', (e) => {
      e.preventDefault();
      pickColor(e.touches[0]);
    });
    canvas.addEventListener('touchmove', (e) => {
      e.preventDefault();
      throttledPick(e.touches[0]);
    });
  ...
  // Fire a fire-and-forget lighting command, surfacing failures as a toast.
  LightingHelpersMixin._cmdWithToast = async function (command, data) {
    try {
      await this.apiClient.sendCommand(command, data);
    } catch (error) {
  ...
  LightingHelpersMixin.testDevice = async function () {
    ...
    await this._cmdWithToast('lighting_device_test', { id: this.selectedDeviceId });
  };

  LightingHelpersMixin.testRule = async function (ruleId) {
    await this._cmdWithToast('lighting_rule_test', { id: ruleId });
  };

  LightingHelpersMixin.allOff = async function () {
    await this._cmdWithToast('lighting_all_off');
  };

  LightingHelpersMixin.blackout = async function () {
    await this._cmdWithToast('lighting_blackout');
  };
  ...
  LightingHelpersMixin._onMasterDimmerChange = async function (value) {
    ...
    try {
      await this.apiClient.sendCommand('lighting_master_dimmer', { value: val });
    } catch (error) {
