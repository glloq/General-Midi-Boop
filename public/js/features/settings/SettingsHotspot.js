(function () {
  ...
  SettingsHotspot.hydrateHotspot = async function () {
    ...
    try {
      ...
      pwdEl.placeholder = cfg.hasPassword
        ? i18n.t('settings.hotspot.passwordPlaceholder') ||
          'Laisser vide pour conserver le mot de passe actuel'
        : i18n.t('settings.hotspot.passwordEmpty') ||
          'Aucun mot de passe enregistré — saisir un mot de passe (8+ caractères)';
    } catch (err) {
    ...
    try {
      ...
      if (active) {
        ...
        const wifiSuffix = wifi
          ...
          : '';
        labelEl.textContent =
          (i18n.t('settings.hotspot.statusInactive') || 'État : hotspot inactif') + wifiSuffix;
        btnEl.textContent = '📡 ' + (i18n.t('settings.hotspot.enable') || 'Activer le hotspot');
        btnEl.dataset.action = 'enable';
      }
    } catch (err) {
      labelEl.textContent =
        (i18n.t('settings.hotspot.statusUnavailable') || 'État indisponible') +
        ': ' +
        (err.message || err);
      btnEl.dataset.action = 'enable';
    }
  ...
  SettingsHotspot._persistHotspotForm = async function () {
    ...
    // Empty field + no stored password → omit password entirely (backend
    // rejects enable() with a clear message). Empty field + stored
    // password → send the sentinel so the backend keeps the existing one.
  ...
  // Shared message banner renderer for both the hotspot (#hotspotMessage)
  // and WiFi-client (#wifiMessage) panels — same kind→colour mapping.
  SettingsHotspot._setPanelMessage = function (selector, kind, text) {
    ...
    const [bg, color] = colors[kind] || colors.ok;
  ...
  SettingsHotspot._showHotspotMessage = function (kind, text) {
    this._setPanelMessage('#hotspotMessage', kind, text);
  };
  ...
  SettingsHotspot.onHotspotToggle = async function () {
    ...
    try {
      if (action === 'enable') {
        ...
        this._showHotspotMessage(
          'ok',
          i18n.t('settings.hotspot.disabledOk') ||
            'Hotspot désactivé, WiFi en cours de reconnexion.'
        );
      }
    } catch (err) {
      ...
      const isExpectedDrop =
        action === 'enable' && /websocket|connection|closed|disconnected|timeout/i.test(msg);
      if (isExpectedDrop) {
        this._showHotspotMessage(
          'warn',
  ...
  SettingsHotspot._showWifiMessage = function (kind, text) {
    this._setPanelMessage('#wifiMessage', kind, text);
  };
  ...
  SettingsHotspot.refreshWifiCurrent = async function () {
    ...
    try {
      ...
      if (wifi) {
        labelEl.innerHTML =
          '✅ ' +
          (i18n.t('settings.wifi.currentConnected') || 'Connecté à') +
          ' <strong>' +
          this._escapeHtml(wifi) +
          '</strong>';
      ...
      /* status failure already surfaced by the hotspot panel */
    }
  };
  ...
  SettingsHotspot.connectWifi = async function (ssid, secured) {
    ...
    if (secured) {
      const ask =
        typeof window.showPrompt === 'function'
          ? window.showPrompt(
              (i18n.t('settings.wifi.passwordPromptMessage') || 'Mot de passe pour') +
                ' "' +
                ssid +
                '"',
              {
    ...
    this._hideWifiMessage();
    this._showWifiMessage(
      'warn',
      '⏳ ' + (i18n.t('settings.wifi.connecting') || 'Connexion en cours à') + ' "' + ssid + '"...'
    );
    ...
    try {
      ...
      this._showWifiMessage(
        'ok',
        '✅ ' + (i18n.t('settings.wifi.connectedOk') || 'Connecté à') + ' "' + ssid + '"'
      );
  ...
  SettingsHotspot.disconnectWifi = async function () {
    ...
    if (!api || !api.sendCommand) return;
    const confirmed = await window.showConfirm(
      i18n.t('settings.wifi.confirmDisconnect') ||
        'Déconnecter le WiFi ? Si vous accédez à cette page via WiFi, vous perdrez la connexion.',
      {
  ...
  // Hook the WiFi-client refresh into hotspot hydration so opening the
  // modal populates everything in one shot.
  const _hydrateHotspotConfig = SettingsHotspot.hydrateHotspot;
  SettingsHotspot.hydrateHotspot = async function () {
    await _hydrateHotspotConfig.call(this);
