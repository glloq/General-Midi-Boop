(function () {
  ...
  SettingsUpdate.triggerSystemUpdate = async function (type) {
    ...
    const confirmed = await window.showConfirm(
      i18n.t('settings.update.confirmMessage') ||
        "Le système va télécharger la dernière version, mettre à jour les dépendances et redémarrer le serveur.\n\nL'application sera temporairement indisponible pendant la mise à jour.",
      {
        title: confirmTitle,
    ...
    statusEl.textContent =
      i18n.t('settings.update.running') || 'Mise à jour en cours, veuillez patienter...';
  ...
  SettingsUpdate._showUpdateInModal = function () {
    ...
    if (!tipBanner && modalInner) {
      ...
      tipBanner.style.cssText =
        'margin: -20px -20px 16px -20px; padding: 14px 20px; background: #fef3c7; color: #92400e; border-radius: 12px 12px 0 0; font-size: 13px; text-align: center; line-height: 1.5;';
      const cacheWarning =
        i18n.t('settings.update.cacheWarning') ||
        'Remember to clear browser cache after the update';
      const cacheWarningHint =
        i18n.t('settings.update.cacheWarningHint') || '(Ctrl+Shift+R or browser settings)';
    ...
    const poll = async () => {
      ...
      try {
          ...
          if (data.status) {
            ...
            if (step === 'restarting') {
              ...
              if (serverWasDown || stuckDuration > 20000) {
                console.log(
                  '[SystemUpdate] Status stuck on "restarting" — server is back, triggering reload',
                  '(serverWasDown:',
                  serverWasDown,
                  'stuckMs:',
                  stuckDuration,
        ...
        if (this._updateModalRefs) {
          ...
          const timeStr =
            mins > 0 ? `${mins}m${secs.toString().padStart(2, '0')}s` : `${elapsedSec}s`;
  ...
  SettingsUpdate._doCacheClearAndReload = function () {
    ...
    try {
      localStorage.setItem('gmboop_update_completed', Date.now());
    } catch (e) {}
  ...
  SettingsUpdate.checkForUpdates = async function () {
    ...
    const STATUS_CSS =
      'padding: 8px 12px; border-radius: 6px; font-size: 12px; display: flex; align-items: center; gap: 8px;';
    const LOADING_CSS =
      STATUS_CSS + ' background: var(--bg-tertiary, #f3f4f6); color: var(--text-secondary, #666);';
    ...
    if (stableChannel) {
      stableChannel.style.display = 'none';
      stableChannel.style.borderColor = '';
      stableChannel.style.boxShadow = '';
    }
    ...
    try {
      ...
      const beta =
        ...
        (!stable.upToDate && !stable.majorMinorChanged
          ? {
              upToDate: false,
              behindCount: stable.behindCount,
              remoteHash: stable.remoteHash,
              branch: result.currentBranch || 'main'
            }
          : null);
      ...
      if (stable.majorMinorChanged) {
        ...
        if (headerBanner) {
          ...
          if (headerBtn)
            headerBtn.textContent = `📦 ${i18n.t('settings.update.stableButton') || 'Installer stable'}`;
