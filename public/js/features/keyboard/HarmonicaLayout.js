...
(function () {
  ...
  function richterKeyed(lo, hi, keyOff) {
    ...
    return blow.length ? { blow, draw } : { blow: [anchor], draw: [anchor] };
  }
    ...
    for (let g = 0; ; g++) {
      for (let k = 0; k < 4; k++) {
        ...
        if (Number.isFinite(hi) && (b > hi || d > hi)) {
          return blow.length ? { blow, draw } : { blow: [root], draw: [root] };
        }
        blow.push(b);
        draw.push(d);
      }
  ...
  function computeLayout(cfg, range) {
    ...
    const count = rng && Array.isArray(rng.notes) && rng.notes.length ? rng.notes.length : null;
    if (count != null) {
      const holes = holesFromCount(count);
      return chromatic ? chromaticCount(lo, keyOff, holes) : richterCount(lo, keyOff, holes);
    }
    return chromatic ? chromaticSolo(lo, hi, keyOff) : richterKeyed(lo, hi, keyOff);
  }
