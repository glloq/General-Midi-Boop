...
(function () {
  ...
  KeyboardWindMixin._initWindPanel = function () {
    ...
    // Articulation buttons (breath slider removed — QA request)
  ...
  KeyboardWindMixin._showWindControls = function (preset) {
    ...
    if (rangeEl && preset) {
      ...
      rangeEl.innerHTML =
        `<span class="wind-range-full">${rMin}–${rMax}</span>` +
    ...
    if (preset) {
      ...
      this.startNote = Math.max(
        0,
        Math.min(127 - this.visibleNoteCount, center - Math.floor(this.visibleNoteCount / 2))
      );
    }
  ...
  KeyboardWindMixin._clearWindComfortZone = function () {
    ...
    if (!strip) return;
    strip
      .querySelectorAll('.piano-slider-key.wind-comfort-zone, .piano-slider-key.wind-out-of-range')
