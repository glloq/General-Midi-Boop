...
(function () {
  ...
  KeyboardControlsMixin.updateSlidersVisibility = function () {
    ...
    modulationPanel.classList.toggle(
      'slider-hidden',
      !(Array.isArray(supportedCcs) && supportedCcs.includes(1))
    );
    ...
    try {
      ...
      if (saved) {
        ...
        // Fallback: old format (number of keys)
          ...
          this.logger.info(
            `[KeyboardModal] Settings loaded (legacy): ${settings.keyboardKeys} keys`
          );
        }

        if (
          settings.keyboardNotation &&
          ['english', 'solfege', 'midi'].includes(settings.keyboardNotation)
        ) {
  ...
  KeyboardControlsMixin._readableInstrumentLabel = function (o) {
    const isMeaningful = (s) =>
      typeof s === 'string' && s.trim() !== '' && !s.startsWith('virtual_');
    ...
    try {
      ...
      const [devices, capsResp] = await Promise.all([
        ...
        this.backend.sendCommand('instrument_list_capabilities').catch((e) => {
          this.logger?.warn(
            '[KeyboardModal] instrument_list_capabilities failed; ' +
              'custom names / virtual instruments degraded:',
            e
          );
          return null;
        })
      ...
      for (const device of activeDevices) {
        ...
        if (!seenIds.has(key)) {
          ...
          this.logger.debug(
            '[KeyboardModal] ✗ Device skipped (duplicate):',
            device.name,
            '(id:',
            key,
            ')'
          ...
          for (const inst of device.instruments) {
            expandedDevices.push({
              ...
              displayName:
                inst.custom_name ||
                inst.name ||
                this._gmProgramName(inst.gm_program) ||
                device.name,
      ...
      this.logger.info(
        `[KeyboardModal] Loaded ${activeDevices.length} → ${uniqueDevices.length} unique devices → ${expandedDevices.length} instruments`
      );
  ...
  KeyboardControlsMixin._updateListViewControls = function () {
    ...
    const supportedCcs = this._parseSupportedCCs(caps);
    const ccList =
      Array.isArray(supportedCcs) && supportedCcs.length > 0 ? supportedCcs : DEFAULT_CC_LIST;
    ...
    if (ccSelect && isListView) {
      ...
      const velocityLabel =
        typeof this.t === 'function' ? this.t('keyboard.velocity') || 'Vélocité' : 'Vélocité';
