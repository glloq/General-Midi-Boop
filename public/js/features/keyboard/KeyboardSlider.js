...
(function () {
  ...
  KeyboardSliderMixin._updateSlideModeGroupVisibility = function () {
    ...
    const mechanismAllowsBend =
      mechanism === 'string_sliding_fingers' || mechanism === 'independent_fingers';
  ...
  KeyboardSliderMixin._setupStringRowDrag = function (row) {
    ...
    const handsConfig =
      cfg.hands_config && cfg.hands_config.enabled === true ? cfg.hands_config : null;
    ...
    const updateHandFollow = (cursorFret) => {
      ...
      const span =
        typeof this._handEffectiveSpanFrets === 'function'
          ? this._handEffectiveSpanFrets()
          : this._handSpanFrets || 4;
      const maxAnchor =
        typeof this._maxHandAnchorFret === 'function'
    ...
    const onDown = (clientX, target) => {
      ...
      const dotFret = dot && dot.dataset.fret !== undefined ? parseInt(dot.dataset.fret, 10) : NaN;
      ...
      const noteFret = Number.isFinite(dotFret) ? dotFret : Math.max(0, Math.ceil(exactFret));
    ...
    const onMove = (clientX) => {
      ...
      const bend = Math.max(
        -4096,
        Math.min(4096, Math.round((exactFret - anchorExactFret) * 4096))
      );
  ...
  KeyboardSliderMixin._sendPitchBend = function (value) {
    ...
    this.backend
      .sendCommand('midi_send_pitchbend', { deviceId, channel, value })
  ...
  KeyboardSliderMixin._updateStringSlideIndicator = function (row, ratio) {
    ...
    const fretAreaPct =
      (row.getBoundingClientRect().width - nutWidth) / row.getBoundingClientRect().width;
  ...
  KeyboardSliderMixin.initPianoSliderDrag = function (strip) {
    ...
    const updateActiveKey = (note) => {
      strip
        .querySelectorAll('.piano-slider-key.active')
