...
(function () {
  ...
  class AccordionView extends InstrumentView {
    ...
    mount(ctx) {
      ...
      const acfg =
        typeof modal.getAccordionConfig === 'function'
      ...
      this._pal = dark
        ...
        : {
            ...
            zoneBg: 'linear-gradient(135deg,#d6404f 0%,#a3122a 45%,' + '#c4324a 70%,#7d0f20 100%)',
            zoneBorder: '#5e0c18',
      ...
      // keyed by its unique data-key so duplicate-note C-griff
      // twins stay independent. onCellOn/Off reflect the twins
      // (the custom setActiveNotes() below owns that logic).
      this._initCellView({
        root,
        selector: '.accordion-key',
        keyOf: (c) => c.dataset.key || c.dataset.note,
        notesOf: (c) =>
          (c.dataset.notes || c.dataset.note || '').split(',').map((s) => parseInt(s, 10)),
        onCellOn: () => this.setActiveNotes(),
        onCellOff: () => this.setActiveNotes()
      });
    }
    ...
    _zone(contentEl, side) {
      ...
      const radius = side === 'treble' ? '8px 0 0 8px' : side === 'bass' ? '0 8px 8px 0' : '8px';
      z.style.cssText =
    ...
    _bellowsVisual() {
      ...
      b.style.cssText = 'flex:0 0 90px;align-self:stretch;pointer-events:none;';
      return b;
    }
    ...
    _buttonBoard(cls, lo, hi, modal, bg) {
      ...
      wrap.style.cssText =
        'display:flex;gap:5px;align-items:stretch;' + 'justify-content:center;height:100%;';
      const label =
      ...
      for (let c = 0; c < nCols; c++) {
        ...
        const pad = c % 2 ? `padding-top:${STAGGER}px;` : `padding-bottom:${STAGGER}px;`;
        col.style.cssText =
    ...
    _cbaBoard(cls, lo, hi, modal, bg) {
      ...
      wrap.style.cssText =
        'display:flex;gap:5px;align-items:flex-start;' + 'justify-content:center;height:100%;';
      const label =
        typeof modal.getNoteLabel === 'function' ? (n) => modal.getNoteLabel(n) : (n) => String(n);
      RANK_OFFSETS.forEach((off, ri) => {
        ...
        const pad = ri % 2 ? `padding-top:${STAGGER}px;` : `padding-bottom:${STAGGER}px;`;
        col.style.cssText =
          'display:flex;flex-direction:column;gap:6px;' +
          'align-items:center;height:100%;width:42px;' +
          'box-sizing:border-box;' +
    ...
    _stradellaGrid(cls, modal, acfg) {
      ...
      const cols = Number.isInteger(ci) && ci >= 1 && ci <= 20 ? ci : STRADELLA_DEFAULT_COLS;
      const bb = Number(acfg.bass_base);
      const base = Number.isInteger(bb) && bb >= 0 && bb <= 127 ? bb : STRADELLA_DEFAULT_BASE;
      ...
      funcs.forEach((id, fi) => {
        ...
        const pad = fi % 2 ? `padding-top:${STAGGER}px;` : `padding-bottom:${STAGGER}px;`;
        colEl.style.cssText =
          'display:flex;flex-direction:column;gap:6px;' +
          'align-items:center;height:100%;width:42px;' +
          'box-sizing:border-box;' +
        ...
        for (let step = 0; step < cols; step++) {
          ...
          if (f.kind === 'note') {
            ...
            notes = f.iv.map((iv) => clampNote(chordLow + mod12(pc + iv)));
            title = label(clampNote(chordLow + pc)) + f.label;
          }
          colEl.appendChild(
            this._mkRound(notes, title, (this._pal && this._pal.bass) || '#3a2b3a', modal)
          );
        }
    ...
    _pianoRow(cls, lo, hi, modal) {
      ...
      col.style.cssText = 'position:relative;width:168px;flex:0 0 auto;' + 'height:100%;';
      let wIdx = 0;
      const mk = (n, black, topPct, hPct, css) => {
        ...
        b.style.cssText =
          ...
          (black
            ? 'background:#1a1a1a;color:#eee;z-index:2;'
      ...
      for (let n = lo; n <= hi; n++) {
        ...
        if (!black) {
          ...
          col.appendChild(mk(n, true, wIdx * H - H * 0.3, H * 0.6, 'right:0;width:62%;'));
        }
      }
      return col;
    }
    ...
    setActiveNotes(_activeMidiSet) {
      ...
      if (this._pressed) {
        for (const { cell, notes } of this._pressed.values()) {
          if (cell && cell.closest && cell.closest('.accordion-treble')) {
      ...
      this._root.querySelectorAll('.accordion-key').forEach((cell) => {
        ...
        if (!on && trebleNotes.size && cell.closest('.accordion-treble')) {
