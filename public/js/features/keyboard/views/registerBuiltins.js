...
(function () {
  ...
  const DRUM_TYPES = new Set(['drum', 'drums', 'drumkit', 'drum_kit', 'percussion', 'percussive']);
  const inRange = (lo, hi) => (c) =>
    c &&
    c.gm_program !== undefined &&
    c.gm_program !== null &&
    c.gm_program >= lo &&
    c.gm_program <= hi;
  const isOneOf = (set) => (c) =>
    c && c.gm_program !== undefined && c.gm_program !== null && set.has(c.gm_program);
  ...
  registry
    ...
    .addRule(
      (c) =>
        c &&
        typeof c.instrument_type === 'string' &&
        DRUM_TYPES.has(c.instrument_type.toLowerCase()),
      'drumpad'
    )
    .addRule(
      (c) => c && c.gm_program !== undefined && c.gm_program !== null && c.gm_program >= 128,
      'drumpad'
    )
    ...
    .addRule(
      (c) =>
        c &&
        typeof c.instrument_type === 'string' &&
        c.instrument_type.toLowerCase() === 'theremin',
      'theremin'
    )
