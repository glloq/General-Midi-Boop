...
(function () {
  ...
  class MusicBoxView extends InstrumentView {
    ...
    mount(ctx) {
      ...
      const label =
        typeof modal.getNoteLabel === 'function' ? (m) => modal.getNoteLabel(m) : (m) => String(m);
      ...
      teeth.style.cssText =
        'flex:1 1 auto;display:flex;align-items:stretch;' + 'gap:1px;min-height:0;';

      const STEEL =
        'linear-gradient(90deg,#5f666e 0,#aab2bb 18%,' +
        '#eef2f6 44%,#cfd6de 60%,#878e96 80%,#5b6068 100%)';
      const BASE = 'linear-gradient(#454b54,#2c3037 55%,#191c21)';
      const STEEL_MAX = 86,
        STEEL_MIN = 42; // % of column height
      ...
      for (let i = 0; i < n; i++) {
        ...
        if (modal.showNoteColors && typeof modal.getNoteColor === 'function') {
          ...
          tooth.style.backgroundImage = `linear-gradient(${noteColor.bg},${noteColor.bg}),${BASE}`;
        }
        ...
        lbl.className = 'music-box-label' + (isOctave ? ' is-octave' : '');
        lbl.textContent = label(midi);
        lbl.style.cssText =
          'position:relative;z-index:1;' +
          'color:' +
          (isOctave ? '#ffffff' : '#cdd4dd') +
          ';' +
      ...
      this._initCellView({
        root,
        selector: '.music-box-tooth',
        onCellOn: (c) => {
          c.style.filter = 'brightness(1.35)';
          c.style.transform = 'scaleY(.96)';
        },
        onCellOff: (c) => {
          c.style.filter = '';
          c.style.transform = '';
