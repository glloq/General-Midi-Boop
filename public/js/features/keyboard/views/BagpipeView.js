...
(function () {
  ...
  class BagpipeView extends InstrumentView {
    ...
    mount(ctx) {
      ...
      const bcfg =
        typeof modal.getBagpipeConfig === 'function'
      ...
      this._onDroneClick = (e) => {
        const closest = e.target && e.target.closest ? (s) => e.target.closest(s) : () => null;
      ...
      // independently of the chanter). Keyed by hole index.
      this._initCellView({
        root,
        selector: '.bagpipe-hole',
    ...
    _syncDroneBtns() {
      ...
      const active = this._drones ? this._drones.filter((d) => d.on).length : 0;
      if (this._droneMaster) {
        ...
        this._droneMaster.textContent = `${active > 0 ? '🟢' : '⚪'} ${allLbl} (${active}/${total})`;
      }
      if (this._droneBtns) {
    ...
    rerender() {
      const wasOn = Array.isArray(this._drones) ? this._drones.map((d) => !!d.on) : null;
      super.rerender();
      if (wasOn && Array.isArray(this._drones)) {
        wasOn.forEach((on, idx) => {
          if (on) this._setDrone(idx, true);
        });
      }
    }
  }
