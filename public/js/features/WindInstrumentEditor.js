    ...
    requestAnimationFrame(() => {
      requestAnimationFrame(() => {
        ...
        if (
          this.renderer &&
          this.melodyCanvasEl &&
          this.melodyCanvasEl.width > 0 &&
          this.melodyCanvasEl.height > 0
        ) {
    ...
    if (this.modal.pianoRoll) {
      ...
      if (pr.xrange && this.melodyCanvasEl.width) {
        this.renderer.ticksPerPixel =
          pr.xrange / (this.melodyCanvasEl.width - this.renderer.headerWidth);
      }
      if (pr.xoffset) {
  ...
  _syncToMidi() {
    ...
    try {
      ...
      const newNotes = this.melodyEvents.map((evt) => {
        const artDef =
          WindInstrumentDatabase.ARTICULATION_TYPES[evt.articulation || 'normal'] ||
