  ...
  class LoopManagerLibraryFeature {
    ...
    async loadLibrary() {
      try {
        ...
        if (typeof this.callbacks.onLibraryLoaded === 'function') {
          try {
            this.callbacks.onLibraryLoaded();
          } catch (_) {
            /* best-effort */
          }
        }
    ...
    filterAndRender() {
      ...
      items.sort((a, b) => {
        ...
        if (this.sort === 'instrument')
          return (a.instrument_program ?? 0) - (b.instrument_program ?? 0);
    ...
    _populateInstrumentFilter() {
      ...
      if (!sel) return;
      const programs = [...new Set(this.library.map((l) => l.instrument_program ?? 0))].sort(
        (a, b) => a - b
      );
