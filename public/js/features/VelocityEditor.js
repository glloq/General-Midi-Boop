...
class VelocityEditor extends BaseLaneEditor {
  ...
  _idOf(item) {
    return item ? item.__index : undefined;
  }
  ...
  handleMouseMove(e) {
    ...
    if (this.isDrawing && this.currentTool === 'draw') {
      ...
      if (
        ...
        Math.abs(snappedTicks - this.lastDrawTicks) >= this.options.grid
      ) {
        const noteAtDraw = this.getNoteAtTick(ticks);
  ...
  render() {
    ...
    if (this.lineStart && this.lastMouseX !== undefined) {
      ...
      this.ctx.beginPath();
      this.ctx.moveTo(this.ticksToX(this.lineStart.ticks), this._valueToY(this.lineStart.velocity));
