    ...
    for (let i = 0; i < this.tabEvents.length; i++) {
      if (this.tabEvents[i].tick >= atTick) {
        ...
        for (const clipEvt of this._clipboard) {
          if (
            this.tabEvents[i].tick === clipEvt.tick + atTick &&
  ...
  _drawTabEvents(w, _h) {
    ...
    for (let i = 0; i < this.tabEvents.length; i++) {
      ...
      if (isHovered) {
        ...
        ctx.fillRect(x - textWidth / 2 - padding - 2, y - 10, textWidth + (padding + 2) * 2, 20);
      }
      ...
      if (warnLevel) {
        ...
        ctx.lineWidth = 1.5;
        ctx.strokeRect(x - textWidth / 2 - padding - 1, y - 9, textWidth + padding * 2 + 2, 18);
        ctx.lineWidth = 1;
      }
  ...
  _handleMouseUp(e) {
    if (this._isDragging) {
      if (this._dragMode === 'select' && this.selectionRect) {
        ...
        const deltaString =
          (currentString > 0 ? currentString : this._moveStartString) - this._moveStartString;
