...
(function () {
  ...
  class LoopManagerArrangerFeature {
    ...
    constructor(modal) {
      ...
      this.view =
        typeof LoopManagerArrangerView !== 'undefined' ? new LoopManagerArrangerView(this) : null;
      this.ops =
        typeof LoopManagerArrangerOps !== 'undefined' ? new LoopManagerArrangerOps(this) : null;
    }

    async _initArrangerSynth() {
      if (!this.modal._arrangerSynth)
        this.modal._arrangerSynth = await LoopUtils.createSynth({ initialProgram: 0 });
    }
    ...
    async _purgeEmptyArrangements() {
      try {
        ...
        for (const arr of arrs) {
          ...
          try {
            const detail = await this.modal.api.sendCommand('arrangement_get', {
    ...
    _renderArrList(arrs) {
      ...
      if (!el) return;
      if (!arrs.length) {
        el.innerHTML = `<div class="lc-empty">${this.modal.t('loopCreator.arrangementsEmpty')}</div>`;
        return;
      }
      el.innerHTML = arrs
        .map(
    ...
    async _newArrangementConfirm() {
      const hasContent =
        (this.modal.blocks?.length || 0) > 0 || (this.modal.tracks?.length || 0) > 0;
    ...
    async _newArrangement() {
      try {
        const r = await this.modal.api.sendCommand('arrangement_create', {
          ...
          global_tempo: this.modal.arrangementTempo,
          total_bars: this.modal.arrangementBars
        });
    ...
    _snapshotArr() {
      return this.view?._snapshotArr();
    }
    _resetArrHistory() {
      return this.view?._resetArrHistory();
    }
    _pushArrHistory() {
      return this.view?._pushArrHistory();
    }
    _arrUndo() {
      return this.view?._arrUndo();
    }
    _arrRedo() {
      return this.view?._arrRedo();
    }
    _restoreArrSnapshot(snap) {
      return this.view?._restoreArrSnapshot(snap);
    }
    _markArrDirty(dirty) {
      return this.view?._markArrDirty(dirty);
    }
    _refreshUndoButtons() {
      return this.view?._refreshUndoButtons();
    }
    _renderPalette() {
      return this.view?._renderPalette();
    }
    _renderTimeline() {
      return this.view?._renderTimeline();
    }
    _renderMinimap() {
      return this.view?._renderMinimap();
    }
    _renderRuler() {
      return this.view?._renderRuler();
    }
    _renderArrangerStartMarker() {
      return this.view?._renderArrangerStartMarker();
    }
    _renderTracks() {
      return this.view?._renderTracks();
    }
    _buildTrackEl(track) {
      return this.view?._buildTrackEl(track);
    }
    _isTrackAudible(trackId) {
      return this.view?._isTrackAudible(trackId) ?? true;
    }
    _toggleTrackMute(trackId) {
      return this.view?._toggleTrackMute(trackId);
    }
    _toggleTrackSolo(trackId) {
      return this.view?._toggleTrackSolo(trackId);
    }
    _buildCells(trackId, BAR_W) {
      return this.view?._buildCells(trackId, BAR_W);
    }
    _toggleBlockSelection(blockId, additive) {
      return this.view?._toggleBlockSelection(blockId, additive);
    }
    _clearBlockSelection() {
      return this.view?._clearBlockSelection();
    }
    _refreshBlockSelectionUI() {
      return this.view?._refreshBlockSelectionUI();
    }
    async _deleteSelectedBlocks() {
      return this.view?._deleteSelectedBlocks();
    }
    _copySelectedBlocks() {
      return this.view?._copySelectedBlocks();
    }
    async _pasteBlocks(targetTrackId, targetBar) {
      return this.view?._pasteBlocks(targetTrackId, targetBar);
    }
    async _duplicateSelectedBlocks() {
      return this.view?._duplicateSelectedBlocks();
    }
    _nextFreeBar(trackId) {
      return this.view?._nextFreeBar(trackId) ?? 0;
    }
    ...
    _showDropPreview(cells, bar, barW) {
      ...
      if (bar + widthBars > this.modal.arrangementBars)
        preview.classList.add('la-drop-preview--overflow');
      cells.appendChild(preview);
      this.modal._dropPreview = preview;
    }

    _hideDropPreview() {
      this.modal._dropPreview?.remove();
      this.modal._dropPreview = null;
    }
    ...
    async _addTrack() {
      return this.ops?._addTrack();
    }
    async _deleteTrack(trackId) {
      return this.ops?._deleteTrack(trackId);
    }
    async _addBlock(trackId, loopId, positionBar, loopBars) {
      return this.ops?._addBlock(trackId, loopId, positionBar, loopBars);
    }
    async _moveBlock(blockId, newTrackId, newPositionBar) {
      return this.ops?._moveBlock(blockId, newTrackId, newPositionBar);
    }
    async _changeReps(blockId, delta) {
      return this.ops?._changeReps(blockId, delta);
    }
    async _deleteBlock(blockId) {
      return this.ops?._deleteBlock(blockId);
    }
    async _saveArrangement(opts) {
      return this.ops?._saveArrangement(opts);
    }
    async _deleteArrangement(id) {
      return this.ops?._deleteArrangement(id);
    }
    async _duplicateArrangement(sourceId) {
      return this.ops?._duplicateArrangement(sourceId);
    }
    _adjustArrTempo(d) {
      return this.ops?._adjustArrTempo(d);
    }
    _adjustArrBars(d) {
      return this.ops?._adjustArrBars(d);
    }
    async _playArrangement(startBar = 0) {
      return this.ops?._playArrangement(startBar);
    }
    _scheduleCountIn(target, secPerBar) {
      return this.ops?._scheduleCountIn(target, secPerBar);
    }
    _stopArrangerPlay() {
      return this.ops?._stopArrangerPlay();
    }
    _startPlaybarRAF() {
      return this.ops?._startPlaybarRAF();
    }
    _stopPlaybarRAF() {
      return this.ops?._stopPlaybarRAF();
    }
    _renderArrangerPlayhead(elapsedSec) {
      return this.ops?._renderArrangerPlayhead(elapsedSec);
    }
    _onDocMouseMove(e) {
      return this.ops?._onDocMouseMove(e);
    }
    async _onDocMouseUp() {
      return this.ops?._onDocMouseUp();
    }
    _scheduleAutoSave(delayMs = 800) {
      return this.ops?._scheduleAutoSave(delayMs);
    }
  }
