...
class CCPitchbendEditorRenderer {
  ...
  render() {
    ...
    if (this.parent.selectionStart && this.parent.lastMouseX !== undefined) {
      ...
      this.parent.ctx.strokeRect(
        this.parent.selectionStart.x,
        this.parent.selectionStart.y,
    ...
    if (this.parent.lineStart && this.parent.lastMouseX !== undefined) {
      ...
      for (let i = 0; i <= segments; i++) {
        ...
        const value =
          this.parent.lineStart.value + (endValue - this.parent.lineStart.value) * curveT;
  ...
  renderGridToBuffer() {
    ...
    const values =
      this.parent.currentCC === 'pitchbend' ? [-8192, -4096, 0, 4096, 8191] : [0, 32, 64, 96, 127];
    ctx.strokeStyle = isDark ? '#3a3a3a' : '#d4daff';
    ctx.lineWidth = 1;
