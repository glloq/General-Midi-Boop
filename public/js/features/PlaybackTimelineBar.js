...
class PlaybackTimelineBar {
  ...
  setHandWarnings(warnings) {
    if (!Array.isArray(warnings)) {
      ...
      this.handWarnings = warnings
        .filter(
          (w) => w && Number.isFinite(w.tick) && (w.level === 'warning' || w.level === 'infeasible')
        )
        .map((w) => ({
  ...
  /**
   * Apply an in-progress marker drag (playhead / rangeStart / rangeEnd)
   * to canvas x. Shared by mouse and touch move handlers.
   * @private
   */
  _applyMarkerDrag(x) {
    const snapped = this._snapToBeat(Math.max(0, this._xToTick(x)));
  ...
  /**
   * Apply an in-progress view pan to canvas x. Shared by mouse and
   * touch move handlers.
   * @private
   */
  _applyPan(x) {
    const tickDelta = (x - this._panStartX) * this.ticksPerPixel;
    this.scrollX = Math.max(0, this._panStartScrollX - tickDelta);
    this._dirty = true;
    this._scheduleRender();
  ...
  _handleTouchStart(e) {
    ...
    const now = Date.now();
    const withinWindow =
      now - this._lastClickTime < this._DOUBLE_CLICK_MS &&
  ...
  _draw() {
    ...
    this._drawHandWarningMarkers(ctx);
  ...
  _drawHandWarningMarkers(ctx) {
    ...
    for (const w of this.handWarnings) {
      ...
      if (
        x < this.leftOffset - size ||
        x > this.canvas.width / (window.devicePixelRatio || 1) + size
      )
        continue;
