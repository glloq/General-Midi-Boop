      ...
      offsetInput.addEventListener('change', () => {
        ...
        if (Number.isFinite(n)) {
          localStorage.setItem(
            'calibration_measurement_offset',
  ...
  async _loadInstruments() {
    ...
    try {
      ...
      const devices = allDevices.filter(
        (d) => d.output !== false && (d.status === 2 || d.connected)
      );
      ...
      for (const device of devices) {
        if (device.instruments && device.instruments.length > 0) {
          for (const inst of device.instruments) {
            ...
            const name =
              inst.custom_name || inst.name || device.displayName || device.name || device.id;
  ...
  async _loadAlsaDevices() {
    try {
      ...
      if (response.success && response.devices && response.devices.length > 0) {
        ...
        if (select) {
          select.innerHTML = response.devices
            .map(
              (device) =>
  ...
  async _measureInstrument(instrument) {
    ...
    try {
      ...
      if (corrected.success) {
        ...
        this._setInstrumentStatus(
          instrument.key,
          'error',
          corrected.error || this.t('calibration.statusError')
        );
      }
