...
(function () {
  ...
  class LoopManagerArrangerOps {
    ...
    async _saveArrangement({ silent = false } = {}) {
      this.modal.arrangementName =
        this.modal.$('#la-name-input')?.value?.trim() ||
        this.modal.t('loopCreator.untitledArrangement');
      const tempo = LoopUtils.validate.tempo(
        this.modal.$('#la-tempo')?.value,
        this.modal.arrangementTempo
      );
      const bars = LoopUtils.validate.arrBars(
        this.modal.$('#la-bars')?.value,
        this.modal.arrangementBars
      );
      try {
    ...
    async _deleteArrangement(id) {
      try {
        ...
        if (this.modal.currentArrangementId === id) await this.parent._newArrangement();
        await this.parent._loadArrangements();
      } catch (err) {
    ...
    async _duplicateArrangement(sourceId) {
      if (this.modal._arrDirty && !confirm(this.modal.t('loopManager.confirmSwitchArrangement')))
        return;
      try {
        ...
        const created = await this.modal.api.sendCommand('arrangement_get', {
          arrangementId: newId
        });
    ...
    _adjustArrTempo(d) {
      ...
      const el = this.modal.$('#la-tempo');
      if (el) el.value = this.modal.arrangementTempo;
    ...
    _adjustArrBars(d) {
      ...
      const el = this.modal.$('#la-bars');
      if (el) el.value = this.modal.arrangementBars;
    ...
    async _playArrangement(startBar = 0) {
      ...
      this.modal._arrangerStartBar = Math.max(
        0,
        Math.min(this.modal.arrangementBars - 1, startBar | 0)
      );
      ...
      if (target) {
        for (const prog of programsToLoad) {
          if (prog >= 128) {
            ...
            await target
              .loadDrumKit?.()
              .catch((err) => LoopUtils.handleError(err, 'arr.synth.loadDrumKit'));
          } else if (!target.loadedInstruments?.has(prog)) {
            await target
              .loadInstrument(prog)
              .catch((err) => LoopUtils.handleError(err, 'arr.synth.loadInstrument'));
          }
        }
      }
      ...
      for (const prog of programsToLoad) {
        ...
        try {
          ...
          LoopUtils.handleError(err, 'arr.synth.setChannelInstrument');
        }
      }
      ...
      for (const [trackId, ch] of trackChMap) {
        ...
        try {
          ...
          LoopUtils.handleError(err, 'arr.synth.setChannelInstrument.track');
        }
      }
      ...
      const scheduleEvents = (offsetMs) => {
        for (const ev of events) {
          ...
          this.modal._arrangerTimers.push(
            setTimeout(
              () => {
                if (!this.modal.isArrangerPlaying) return;
                try {
                  target?.playNote?.(ev.note, ev.vel, ch, ev.durSec);
                } catch (err) {
                  LoopUtils.handleError(err, 'arr.synth.playNote');
                }
              },
          ...
          this.modal._arrangerTimers.push(
            setTimeout(
              () => {
                if (!this.modal.isArrangerPlaying) return;
                try {
                  target?.setChannelInstrument?.(9, drumProg);
                } catch (err) {
                  LoopUtils.handleError(err, 'arr.restoreDrumProg');
                }
              },
      ...
      if (this.modal._arrangerLoop) {
        ...
        this.modal._arrangerTimers.push(
          setTimeout(() => this._stopArrangerPlay(), countInMs + playableMs)
        );
      }
    ...
    _scheduleCountIn(target, secPerBar) {
      ...
      const ch = 9;
      try {
        target?.setChannelInstrument?.(ch, 115);
      } catch (err) {
        LoopUtils.handleError(err, 'arr.countIn.setProgram');
      }
      ...
      for (let i = 0; i < 4; i++) {
        this.modal._arrangerTimers.push(
          setTimeout(() => {
            if (!this.modal.isArrangerPlaying) return;
            try {
              target?.playNote?.(76, 100, ch, 0.08);
            } catch (err) {
              LoopUtils.handleError(err, 'arr.countIn.playNote');
            }
          }, i * beatMs)
        );
      }
    ...
    _stopArrangerPlay() {
      ...
      this.modal.$('#la-play-btn')?.classList.remove('lc-btn-record--active');
      try {
        this.modal._arrangerSynth?.cancelAllNotes?.();
      } catch (err) {
        LoopUtils.handleError(err, 'arr.synth.cancelAllNotes');
      }
      try {
        this.modal._deviceShim?.cancelAllNotes?.();
      } catch (err) {
        LoopUtils.handleError(err, 'arr.device.cancelAllNotes');
      }
    ...
    _stopPlaybarRAF() {
      if (this.modal._playbarRAF) {
        cancelAnimationFrame(this.modal._playbarRAF);
        this.modal._playbarRAF = null;
      }
    }
    ...
    _renderArrangerPlayhead(elapsedSec) {
      ...
      const bar = Math.min(
        this.modal.arrangementBars,
        this.modal._arrangerStartBar + elapsedSec / secPerBar
      );
    ...
    _onDocMouseMove(e) {
      ...
      if (nameLabel) {
        ...
        const overflow =
          block && block.position_bar + r.loopBars * newReps > this.modal.arrangementBars;
