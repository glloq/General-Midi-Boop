...
(function () {
  ...
  class LoopManagerLiveFeature {
    ...
    renderArea() {
      ...
      const filtered = q ? m.library.filter((l) => l.name.toLowerCase().includes(q)) : m.library;
                ...
                <div class="lm-live-loops">
                    ${loops
                      .map((l) => {
                        ...
                        const tempoRange =
                          l.tempo < 90 ? 'slow' : l.tempo < 140 ? 'medium' : 'fast';
                        ...
                        const tempoLabel =
                          m.t('loopManager.tempoRange_' + tempoRange) || tempoRange;
    ...
    async trigger(loopId) {
      ...
      if (target) {
        try {
          ...
          LoopUtils.handleError(err, 'live.synth.setChannelInstrument');
        }
        if (isDrum) {
          await target
            .loadDrumKit?.()
            .catch((err) => LoopUtils.handleError(err, 'live.synth.loadDrumKit'));
        } else if (!target.loadedInstruments?.has(prog)) {
          await target
            .loadInstrument(prog)
            .catch((err) => LoopUtils.handleError(err, 'live.synth.loadInstrument'));
        }
      }

      const loopDurMs = LoopUtils.loopDurationMs(loopData);
      this.playingLoops.set(loopId, {
        timers: [],
        ch,
    ...
    _scheduleLoop(loopId, loopData) {
      ...
      state.timers = LoopUtils.scheduleSequence({
        ...
        cycleMs: loopDurMs,
        onCycleEnd: () => {
          if (isAlive()) this._scheduleLoop(loopId, loopData);
        }
      });
    }
    ...
    stop(loopId) {
      ...
      if (target && state.ch != null) {
        try {
          target.allNotesOff?.(state.ch);
        } catch (err) {
          LoopUtils.handleError(err, 'live.allNotesOff');
        }
        ...
        if (typeof target.allNotesOff !== 'function') {
          try {
            target.cancelAllNotes?.();
          } catch (err) {
            LoopUtils.handleError(err, 'live.cancelAllNotes');
          }
        }
      }
    ...
    stopAll() {
      ...
      try {
        ...
        LoopUtils.handleError(err, 'live.synth.cancelAllNotes');
      }
      try {
        m._deviceShim?.cancelAllNotes?.();
      } catch (err) {
        LoopUtils.handleError(err, 'live.device.cancelAllNotes');
      }
    }
    ...
    _allocChannel() {
      const used = new Set(
        [...this.playingLoops.values()].map((s) => s.ch).filter((c) => c != null)
      );
      for (let c = 0; c < 16; c++) {
