...
class WindMelodyRenderer extends CanvasRenderer {
  ...
  _dispatchSelectionChange() {
    this.canvas.dispatchEvent(
      new CustomEvent('wind:selectionchange', {
  ...
  _renderRangeZones(ctx, w, h) {
    ...
    if (bottomOutH > 0)
      ctx.fillRect(this.headerWidth, bottomOutY, w - this.headerWidth, bottomOutH);
  ...
  _renderNotes(ctx, w, _h) {
    ...
    for (let i = 0; i < this.melodyEvents.length; i++) {
      ...
      const isOutOfRange =
        this.rangeCheckEnabled && (evt.note < this.noteMin || evt.note > this.noteMax);
    ...
    if (art === 'legato') {
      ...
      if (nextIdx < this.melodyEvents.length) {
        ...
        ctx.quadraticCurveTo(
          ...
          y - noteH / 2 - 12,
          nextX + 4,
          this._noteToY(nextEvt.note) - noteH / 2 - 2
        );
        ctx.stroke();
      }
    } else {
      ...
      if (e.shiftKey && this._lastClickedKey !== undefined) {
        ...
        const releaseHandler = () => {
          canvas.dispatchEvent(
            new CustomEvent('wind:pianokeyup', {
              detail: { note },
              bubbles: false
            })
          );
    ...
    if (this._isDragging && this._dragMode === 'pan') {
      ...
      if (noteDelta !== prevNoteDelta) {
        ...
        for (const idx of this.selectedEvents) {
          ...
          if (evt)
            movedNotes.push({
              n: evt.note + noteDelta,
              v: evt.velocity || 100,
              c: evt.channel || 0
            });
        }
        this.canvas.dispatchEvent(
          new CustomEvent('wind:notedragmove', {
            detail: { notes: movedNotes },
            bubbles: false
          })
        );
      }
  ...
  _handleDblClick(e) {
    ...
    if (hitIdx >= 0) {
      ...
      this.canvas.dispatchEvent(
        new CustomEvent('wind:editarticulation', {
