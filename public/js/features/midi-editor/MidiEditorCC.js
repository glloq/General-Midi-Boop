...
(function () {
  ...
  class MidiEditorCC {
    constructor(modal) {
      ...
      this.buttons =
        typeof MidiEditorCCButtons !== 'undefined' ? new MidiEditorCCButtons(this) : null;
    }
    ...
    // Apply a channel to whichever lane editor is current (velocity vs CC).
    _setActiveEditorChannel(channel) {
    ...
    updateEditorChannelSelector() {
      ...
      if (this.modal.currentCCType === 'tempo') {
        ...
        channelSelector.innerHTML = allChannels
          .map(
            (channel) =>
      ...
      if (this.modal.activeChannels && this.modal.activeChannels.size === 1) {
        ...
        this._setActiveEditorChannel(activeChannel);
      ...
      if (channelsToShow.length === 0) {
        const message =
          this.modal.currentCCType === 'velocity'
            ? this.modal.t('midiEditor.noNotesInFile')
            : this.modal.t('midiEditor.noCCInFile');
      ...
      channelSelector.innerHTML = channelsToShow
        .map((channel) => {
      ...
      this.modal.log(
        'info',
        `Sélecteur de canal mis à jour - Type ${this.modal.currentCCType}: ${channelsToShow.length} canaux`
      );
    }
    ...
    attachEditorChannelListeners() {
      ...
      channelSelector.addEventListener('click', (e) => {
        ...
        channelSelector
          .querySelectorAll('.cc-channel-btn')
    ...
    extractCCAndPitchbend() {
      ...
      // Push a normalised CC-style event, filling in the shared
      // channel-default and unique id used by every branch below.
      const pushEvent = (currentTick, event, extra) => {
        ...
        track.events.forEach((event) => {
          ...
          if (event.type === 'controller' || event.type === 'controlChange') {
            ...
            if (controller !== undefined && controller >= 0 && controller <= 127) {
              pushEvent(currentTick, event, { type: `cc${controller}`, value: event.value });
            }
          }

          // Pitch Bend events
          if (event.type === 'pitchBend') {
            pushEvent(currentTick, event, { type: 'pitchbend', value: event.value });
          }

          // Channel Aftertouch events
          if (event.type === 'channelAftertouch') {
            pushEvent(currentTick, event, {
              type: 'aftertouch',
    ...
    updateDynamicCCButtons() {
      return this.buttons?.updateDynamicCCButtons();
    }
    toggleCCSection() {
      return this.buttons?.toggleCCSection();
    }
    selectBestCCTypeForChannel(channel) {
      return this.buttons?.selectBestCCTypeForChannel(channel);
    }
    selectCCType(ccType) {
      return this.buttons?.selectCCType(ccType);
    }
    getUsedCCTypesForChannel(channel) {
      return this.buttons?.getUsedCCTypesForChannel(channel) ?? new Set();
    }
    getAllUsedCCTypes() {
      return this.buttons?.getAllUsedCCTypes() ?? new Set();
    }
    highlightUsedCCButtons() {
      return this.buttons?.highlightUsedCCButtons();
    }
  }
