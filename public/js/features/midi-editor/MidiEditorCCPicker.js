...
(function () {
  ...
  class MidiEditorCCPicker {
    constructor(modal) {
      ...
      this.laneEditors =
        typeof MidiEditorLaneEditors !== 'undefined' ? new MidiEditorLaneEditors(this) : null;
    }
    ...
    updateCCEditorChannel() {
      ...
      const activeChannel =
        this.modal.activeChannels.size > 0 ? Array.from(this.modal.activeChannels)[0] : 0;
    ...
    initCCEditor() {
      return this.laneEditors?.initCCEditor();
    }
    waitForCCEditorLayout(attempts, maxAttempts) {
      return this.laneEditors?.waitForCCEditorLayout(attempts, maxAttempts);
    }
    syncCCEditor() {
      return this.laneEditors?.syncCCEditor();
    }
    _getActiveEditorHeaderWidth() {
      return this.laneEditors?._getActiveEditorHeaderWidth() ?? 0;
    }
    syncAllEditors() {
      return this.laneEditors?.syncAllEditors();
    }
    syncCCEventsFromEditor() {
      return this.laneEditors?.syncCCEventsFromEditor();
    }
    syncTempoEventsFromEditor() {
      return this.laneEditors?.syncTempoEventsFromEditor();
    }
    initVelocityEditor() {
      return this.laneEditors?.initVelocityEditor();
    }
    waitForVelocityEditorLayout(a, m) {
      return this.laneEditors?.waitForVelocityEditorLayout(a, m);
    }
    initTempoEditor() {
      return this.laneEditors?.initTempoEditor();
    }
    waitForTempoEditorLayout(a, m) {
      return this.laneEditors?.waitForTempoEditorLayout(a, m);
    }
    ...
    hideCurveButtons() {
      ...
      if (curveSection) {
        ...
        if (
          curveSection.previousElementSibling &&
          curveSection.previousElementSibling.classList.contains('cc-toolbar-divider')
        ) {
    ...
    syncSequenceFromVelocityEditor(velocitySequence) {
      ...
      this.modal.fullSequence.forEach((note) => {
        const velocityNote = velocitySequence.find(
          (vn) => vn.t === note.t && vn.n === note.n && vn.c === note.c
        );
        if (velocityNote) {
      ...
      channelNoteCount.forEach((count, channel) => {
        ...
        const instrumentName =
          channel === 9
            ...
            : this.modal.getInstrumentName(program);
