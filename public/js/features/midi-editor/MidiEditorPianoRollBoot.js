...
(function () {
  ...
  class MidiEditorPianoRollBoot {
    ...
    async initPianoRoll() {
      ...
      m.log(
        'info',
        `Piano roll configured: xrange=${xrange}, yrange=${noteRange}, yoffset=${yoffset} (centered), tempo=${m.tempo || 120} BPM, timebase=${m.ticksPerBeat || 480} ticks/beat`
      );
      ...
      m.log(
        'info',
        `Piano roll grid/snap: grid=120 ticks, snap=${currentSnap.ticks} ticks (${currentSnap.label})`
      );
