...
class MidiEditorToolbar {
  ...
  cycleSnap() {
    ...
    if (m.pianoRoll) {
      ...
      m.log(
        'info',
        `Snap to grid changed to ${currentSnap.label} (${currentSnap.ticks} ticks) - snap property set to ${m.pianoRoll.snap}`
      );
    }
  ...
  async changeChannel() {
    ...
    if (!targetChannelInfo && instrumentSelector) {
      ...
      m.log(
        'info',
        `New channel ${newChannel} will use instrument: ${m.getInstrumentName(m.selectedInstrument)}`
      );
    }
  ...
  async applyInstrument() {
    ...
    if (result === true && hasSelection) {
      ...
      this.editActions.applyInstrumentToChannel(
        targetChannel,
        selectedProgram,
        instrumentName,
        channelInfo
  ...
  async applyInstrumentToSelection(program, instrumentName) {
    ...
    m.showNotification(
      m.t('midiEditor.instrumentAppliedToSelection', {
  ...
  applyInstrumentToChannel(channel, program, instrumentName, channelInfo) {
    ...
    m.showNotification(
      m.t('midiEditor.instrumentApplied', { channel: channel + 1, instrument: instrumentName }),
      'success'
    );
