...
(function () {
  ...
  class MidiEditorLaneEditors {
    ...
    initCCEditor() {
      ...
      this.modal.log(
        'info',
        `Initializing CC Editor with ${this.modal.ccEvents.length} total CC events`
      );
      ...
      if (this.modal.activeChannels && this.modal.activeChannels.size === 1) {
        ...
        activeChannel =
          fileChannels.length > 0 ? fileChannels[0] : usedChannels.length > 0 ? usedChannels[0] : 0;
      }
      ...
      this.modal.log(
        'info',
        `CC Editor initialized - Type: ${this.modal.currentCCType}, Channel: ${activeChannel + 1}, File channels: [${fileChannels.map((c) => c + 1).join(', ')}]`
      );
    ...
    waitForCCEditorLayout(attempts = 0, maxAttempts = 60) {
      ...
      if (height > 100) {
        ...
        if (this.modal.velocityEditor && typeof this.modal.velocityEditor.resume === 'function')
          this.modal.velocityEditor.resume();
        if (this.modal.tempoEditor && typeof this.modal.tempoEditor.resume === 'function')
          this.modal.tempoEditor.resume();
        this.modal.log(
          'info',
          `CC Editor layout ready after ${attempts} attempts (height=${height})`
        );
        ...
        this.modal.log(
          'error',
          `waitForCCEditorLayout: Max attempts reached (${maxAttempts}), height still ${height}px`
        );
      }
    }
    ...
    syncAllEditors() {
      ...
      if (this.modal.timelineBar) {
        ...
        this.modal.timelineBar.setZoom(
          viewport.xrange / Math.max(1, containerWidth - activeLeftOffset)
        );
      }
    ...
    syncCCEventsFromEditor() {
      if (!this.modal.ccEditor) {
        ...
        this.modal.log(
          'debug',
          `syncCCEventsFromEditor: editor not opened, preserving ${this.modal.ccEvents.length} original events`
        );
        return;
      }
      ...
      this.modal.log(
        'info',
        `Synchronized ${this.modal.ccEvents.length} CC/pitchbend events from editor`
      );
    ...
    syncTempoEventsFromEditor() {
      if (!this.modal.tempoEditor) {
        this.modal.log(
          'info',
          `syncTempoEventsFromEditor: Tempo editor not initialized, keeping ${this.modal.tempoEvents.length} original events`
        );
        return;
      }
      ...
      this.modal.log(
        'info',
        `Synchronized ${this.modal.tempoEvents.length} tempo events from editor`
      );
    }
    ...
    initVelocityEditor() {
      ...
      this.modal.log(
        'info',
        `Initializing Velocity Editor with ${this.modal.sequence.length} notes`
      );
      ...
      const firstChannel =
        this.modal.activeChannels && this.modal.activeChannels.size === 1
          ? Array.from(this.modal.activeChannels)[0]
          : this.modal.channels.length > 0
      ...
      this.modal.log(
        'info',
        `Velocity Editor initialized with ${this.modal.fullSequence.length} notes, default channel: ${firstChannel + 1}`
      );
    ...
    waitForVelocityEditorLayout(attempts = 0, maxAttempts = 60) {
      ...
      if (height > 100) {
        ...
        this.modal.log(
          'info',
          `Velocity Editor layout ready after ${attempts} attempts (height=${height})`
        );
      } else if (attempts < maxAttempts) {
        // Layout is not ready yet, retry on the next frame
        requestAnimationFrame(() => {
        ...
        this.modal.log(
          'error',
          `waitForVelocityEditorLayout: Max attempts reached (${maxAttempts}), height still ${height}px`
        );
      }
    }
    ...
    initTempoEditor() {
      ...
      this.modal.log(
        'info',
        `Tempo Editor initialized with ${this.modal.tempoEvents.length} events`
      );
    ...
    waitForTempoEditorLayout(attempts = 0, maxAttempts = 60) {
      ...
      if (height > 100) {
        ...
        this.modal.log(
          'info',
          `Tempo Editor layout ready after ${attempts} attempts (height=${height})`
        );
      } else if (attempts < maxAttempts) {
        // Layout is not ready yet, retry on the next frame
        requestAnimationFrame(() => {
        ...
        this.modal.log(
          'error',
          `waitForTempoEditorLayout: Max attempts reached (${maxAttempts}), height still ${height}px`
        );
      }
    }
  }
