  ...
  class LoopManagerArrangerView {
    ...
    _renderPalette() {
      ...
      const items = q
        ...
        : this.modal.library;
      if (!items.length) {
        grid.innerHTML = `<div class="lc-empty">${this.modal.t('loopManager.noResults')}</div>`;
        return;
      }
      ...
      if (!grid._laPaletteDnDBound) {
        ...
        grid.addEventListener('dragend', (e) => {
          if (e.target.closest('.la-palette-chip')) this.modal._dragInfo = null;
        });
      }
    }
    ...
    _buildTrackEl(track) {
      ...
      const defaultLabel =
        this.modal.t('loopManager.defaultTrackName', { index: (track.track_index ?? 0) + 1 }) ||
      ...
      nameInput?.addEventListener('change', async () => {
        try {
          await this.modal.api.sendCommand('arrangement_update_track', {
            trackId: track.id,
      ...
      trackEl
        .querySelector('[data-track-action="delete"]')
        ?.addEventListener('click', () => this.parent._deleteTrack(track.id));
      trackEl
        .querySelector('[data-track-action="mute"]')
        ?.addEventListener('click', () => this._toggleTrackMute(track.id));
      trackEl
        .querySelector('[data-track-action="solo"]')
        ?.addEventListener('click', () => this._toggleTrackSolo(track.id));
      ...
      this.modal.blocks
        ...
        .forEach((block) => {
          ...
          const title = overflow
            ? this.modal.t('loopManager.blockOverflow', {
                end: endBar,
                total: this.modal.arrangementBars
              })
    ...
    _toggleBlockSelection(blockId, additive) {
      if (additive) {
        ...
        const onlyOne =
          this.modal._selectedBlocks.size === 1 && this.modal._selectedBlocks.has(blockId);
        this.modal._selectedBlocks.clear();
    ...
    async _pasteBlocks(targetTrackId = null, targetBar = null) {
      ...
      const baseTrackIdx =
        targetTrackId != null ? Math.max(0, tracksOrder.indexOf(targetTrackId)) : 0;
      const baseBar = targetBar != null ? targetBar : this._nextFreeBar(tracksOrder[baseTrackIdx]);
      const newBlocks = [];
      for (const item of this.modal._blockClipboard) {
        ...
        const posBar = Math.max(
          0,
          Math.min(this.modal.arrangementBars - 1, baseBar + item.bar_offset)
        );
        try {
