...
(function () {
  ...
  function renderChannelHistogram(channelAnalysis, transposition = 0) {
    ...
    const r = safeNoteRange(
      channelAnalysis.noteRange.min + transposition,
      channelAnalysis.noteRange.max + transposition
    );
    const chMin = r.min;
    const chMax = r.max;
    const noteCount = chMax - chMin + 1;
  ...
  function renderMiniRange(analysis, assignment) {
    ...
    if (assignment && assignment.noteRangeMin != null) {
      ...
      const iWidth = Math.max(
        2,
        Math.round(((assignment.noteRangeMax - assignment.noteRangeMin) / 127) * 100)
      );
  ...
  function renderInstrumentChips(opts) {
    ...
    const { MAX_INST_NAME, getTypeColor, getScoreClass } = window.RoutingSummaryConstants;
    ...
    const chipHTML = (opt, extraClass = '') => {
      ...
      const displayName =
        name.length > MAX_INST_NAME ? name.slice(0, MAX_INST_NAME - 1) + '\u2026' : name;
      return `
    ...
    const visibleLowOptions =
      showLowChips && lowOptions.length > 0 ? lowOptions.slice(0, LOW_CHIPS_MAX) : [];
    ...
    const showMoreBtn =
      lowOptions.length > 0 && options.length > 0
        ? `
  ...
  function renderPolyReductionSection(opts) {
    const { channel, adaptation, assignment, channelPolyphony, instrumentPolyphony } = opts;
    ...
    const polyTarget =
      polyReduction === 'manual' && adaptation.polyTarget != null
        ? adaptation.polyTarget
        : effectivePoly;
    const polyExcess = channelPolyphony - polyTarget;
    const impactKey =
      polyStrategy === 'shorten' ? 'autoAssign.polyImpactShorten' : 'autoAssign.polyImpactDrop';
  ...
  function renderRangeBars(opts) {
    ...
    const { SPLIT_COLORS, FULL_RANGE, getGmProgramName, midiNoteToName } =
    ...
    if (splitData?.segments?.length > 0) {
      ...
      instBarsHTML = segs
        .map((seg, i) => {
          ...
          const instLookup = seg.instrumentId
            ? allInstruments.find((ii) => ii.id === seg.instrumentId)
            : null;
          const name = instLookup
            ? getDisplayName(instLookup)
            : seg.instrumentName || `Inst ${i + 1}`;
      ...
      const overlapZonesHTML =
        overlaps.length > 0
          ? overlaps
              .map((ov) => {
                const oLeft = (ov.min / FULL_RANGE) * 100;
                const oWidth = Math.max(0.5, ((ov.max - ov.min) / FULL_RANGE) * 100);
                const instA = segs[ov.segA]?.instrumentId
                  ? allInstruments.find((ii) => ii.id === segs[ov.segA].instrumentId)
                  : null;
                const instB = segs[ov.segB]?.instrumentId
                  ? allInstruments.find((ii) => ii.id === segs[ov.segB].instrumentId)
                  : null;
                const nameA = instA
                  ? getDisplayName(instA)
                  : segs[ov.segA]?.instrumentName || `Inst ${ov.segA + 1}`;
                const nameB = instB
                  ? getDisplayName(instB)
                  : segs[ov.segB]?.instrumentName || `Inst ${ov.segB + 1}`;
    ...
    for (let oct = 0; oct <= 10; oct++) {
      ...
      if (note <= 127) {
        ...
        octaveMarkers.push(
          `<span class="rs-range-octave-mark" style="left:${pct}%">C${oct}</span>`
        );
      }
    }
  ...
  function renderDrumMappingSection(opts) {
    ...
    if (!isExpanded) {
      const noteCount = Object.keys(analysis.noteDistribution || {}).filter(
        (n) => +n >= 35 && +n <= 81
      ).length;
      if (noteCount === 0) return '';
      return `
    ...
    const rows = channelNotes
      .map((srcNote) => {
        ...
        if (isMuted) {
          typeLabel = 'Muté';
          typeClass = 'rs-drum-type-muted';
        } else if (isCustom) {
          typeLabel = 'Manuel';
          typeClass = 'rs-drum-type-custom';
        } else if (isExact && isAvailable) {
          typeLabel = 'Exact';
          typeClass = 'rs-drum-type-exact';
        } else if (isSubstitution) {
          typeLabel = 'Subst.';
          typeClass = 'rs-drum-type-subst';
        } else {
          typeLabel = 'N/A';
          typeClass = 'rs-drum-type-na';
        }

        const destOptions = sortedInstrumentNotes
          .map((n) => {
  ...
  function renderCCSection(opts) {
    ...
    if (isSplit && segments.length > 0) {
      const headerCols = segments
        .map((seg, i) => {
          const color = SPLIT_COLORS[i % SPLIT_COLORS.length];
          const instRef = seg.instrumentId
            ? allInstruments.find((ii) => ii.id === seg.instrumentId)
            : null;
      ...
      const bodyRows = visibleCCs
        .map((ccNum) => {
          ...
          if (isDisabled) {
            cells = segments
              .map(() => `<td class="rs-cc-cell rs-cc-cell-disabled">\u2014</td>`)
              .join('');
          } else {
            cells = segCCs
              .map((ccs, i) => {
                ...
                if (ccs === null)
                  return `<td class="rs-cc-cell rs-cc-cell-unknown">${segToggleBtn} ?</td>`;
                if (ccs.includes(ccNum))
                  return `<td class="rs-cc-cell rs-cc-cell-ok">${segToggleBtn} \u2713</td>`;
                const currentTarget = currentRemap[ccNum];
                const remapOpts = (ccs || [])
                  .filter((tc) => !channelCCSet.has(tc) || tc === ccNum)
                  .map(
                    (tc) =>
                      `<option value="${tc}" ${currentTarget === tc ? 'selected' : ''}>${getCCName(tc)}</option>`
                  )
                  .join('');
    ...
    const bodyRows = visibleCCs
      .map((ccNum) => {
        ...
        const rowClass = isDisabled
          ? 'rs-cc-row-disabled'
          : instrumentCCs !== null && !instrumentCCs.includes(ccNum) && !isDisabled
            ? 'rs-cc-row-warn'
            : '';
  ...
  function renderScoreDetail(opts) {
    ...
    const { SPLIT_COLORS, getGmProgramName, midiNoteToName, getScoreClass, getScoreBgClass } =
      window.RoutingSummaryConstants;
    ...
    if (channelKeys.length === 0)
      return `<div class="rs-score-empty">${_t('routingSummary.noChannels') || 'Aucun canal'}</div>`;
    ...
    if (isDetailMode) {
      ...
      if (isSplit && splitAssignments[channel]) {
        ...
        const totalNotes = analysis?.noteDistribution
          ? Object.values(analysis.noteDistribution).reduce((s, c) => s + c, 0)
          : 0;
        ...
        const segRows = segments
          .map((seg, i) => {
            ...
            if (analysis?.noteDistribution) {
              ...
              const semi =
                autoAdaptation && adapt.pitchShift !== 'none'
      ...
      const issues =
        !isSkipped && assignment?.issues?.length
          ? `<div class="rs-score-issues">${assignment.issues
              .map(
                (i) =>
    ...
    const cells = channelKeys
      .map((ch) => {
        ...
        const score = assignment?.score || 0;
        const isShared =
          assignment?.shared || (assignment?.sharedWith && assignment.sharedWith.length > 0);
        const instName = isSkipped
        ...
        const sharedTitle =
          isShared && assignment.sharedWith?.length
  ...
  function renderSummaryTable(opts) {
    ...
    const { SPLIT_COLORS, getGmProgramName, getScoreClass, getTypeIcon, getTypeColor } =
      window.RoutingSummaryConstants;
    ...
    const rows = channelKeys
      .map((ch) => {
        ...
        if (isCondensed) {
          ...
          if (isSkipped) {
            ...
            if (assignment.shared || (assignment.sharedWith && assignment.sharedWith.length > 0)) {
              routedName +=
                ' <span class="rs-shared-badge" title="' +
                escape(_t('routingSummary.sharedTooltip') || 'Instrument partagé') +
                '">\u{1F517}</span>';
            }
          } else {
        ...
        const scoreHTML =
          !isSkipped && score > 0
            ? `<span class="rs-score-value ${getScoreClass(score)}">${score}${sharedBadge}</span>`
            : '';
        ...
        if (!isSkipped) {
          ...
          if (chPoly && instPoly) {
            const adapt = adaptationSettings[ch];
            const polyActive =
              autoAdaptation && adapt?.polyReduction && adapt.polyReduction !== 'none';
            const ok = polyActive || instPoly >= chPoly;
            const polyLabel = polyActive
              ? `${chPoly}\u2192${adapt.polyTarget || instPoly}`
              : `${chPoly}/${instPoly}`;
  ...
  function renderSplitSection(opts) {
    const {
      ...
      semitones,
      allInstruments = [],
      getCompatibleInstrumentsForSegment,
      getDisplayName,
      detectOverlaps,
      escape
    } = opts;
    const { MAX_INST_NAME, SPLIT_COLORS, safeNoteRange, midiNoteToName } =
      window.RoutingSummaryConstants;
    ...
    const chRange = safeNoteRange(
      (analysis?.noteRange?.min ?? 0) + semitones,
      (analysis?.noteRange?.max ?? 127) + semitones
    );
    ...
    const instRowsHTML = segments
      .map((seg, i) => {
        ...
        let segOptionInstruments =
          compatInstruments.length > SEGMENT_SELECT_MAX
            ? compatInstruments.slice(0, SEGMENT_SELECT_MAX)
            : compatInstruments;
        if (
          seg.instrumentId &&
          !segOptionInstruments.some((inst) => inst.id === seg.instrumentId)
        ) {
          const currentInst =
            compatInstruments.find((inst) => inst.id === seg.instrumentId) ||
            allInstruments.find((ii) => ii.id === seg.instrumentId);
          if (currentInst) segOptionInstruments = [currentInst, ...segOptionInstruments];
        }
        const selectOptions = segOptionInstruments
          .map((inst) => {
        ...
        const physWidth = Math.max(
          1,
          Math.round(((displayPhysMax - displayPhysMin + 1) / noteCount) * 100)
        );
