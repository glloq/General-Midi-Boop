...
(function () {
  ...
  class KeyboardPreview {
    ...
    setHandBands(bands) {
      this.handBands = Array.isArray(bands)
        ? bands.filter(
            (b) =>
              b &&
    ...
    _geo() {
      ...
      for (let m = this.rangeMin; m <= this.rangeMax; m++) {
        xOf[m] = isBlackKey(m) ? whiteIdx[m - 1] * ww + ww * 0.65 : whiteIdx[m] * ww;
      }
      this._geoCache = { w, ww, rangeMin: this.rangeMin, rangeMax: this.rangeMax, whiteIdx, xOf };
