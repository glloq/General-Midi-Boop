...
(function () {
  ...
  class ScoringSettingsModal extends BaseModal {
    ...
    static DRUM_CAT_INFO = [
      {
        key: 'kicks',
        icon: '🥁',
        labelKey: 'scoringSettings.catKicks',
        fallback: 'Kicks',
        notes: [35, 36]
      },
      {
        key: 'snares',
        icon: '🪘',
        labelKey: 'scoringSettings.catSnares',
        fallback: 'Snares',
        notes: [37, 38, 40]
      },
      {
        key: 'hiHats',
        icon: '🎩',
        labelKey: 'scoringSettings.catHiHats',
        fallback: 'Hi-Hats',
        notes: [42, 44, 46]
      },
      {
        key: 'toms',
        icon: '🥁',
        labelKey: 'scoringSettings.catToms',
        fallback: 'Toms',
      ...
      },
      {
        key: 'crashes',
        icon: '💥',
        labelKey: 'scoringSettings.catCrashes',
        fallback: 'Crashes',
        notes: [49, 52, 55, 57]
      },
      {
        key: 'rides',
        icon: '🔔',
        labelKey: 'scoringSettings.catRides',
        fallback: 'Rides',
        notes: [51, 53, 59]
      },
      {
        key: 'latin',
        icon: '🪇',
        labelKey: 'scoringSettings.catLatin',
        fallback: 'Latin',
      ...
      },
      {
        key: 'shakers',
        icon: '🫧',
        labelKey: 'scoringSettings.catShakers',
        fallback: 'Shakers',
        notes: [39, 54, 58, 69, 70]
      },
      {
        key: 'woodsMetal',
        icon: '🪵',
        labelKey: 'scoringSettings.catWoodsMetal',
        fallback: 'Bois & Métal',
        notes: [56, 75, 76, 77]
      },
      {
        key: 'pitched',
        icon: '🎶',
        labelKey: 'scoringSettings.catPitched',
        fallback: 'Effets mélodiques',
        notes: [71, 72, 73, 74]
      },
      {
        key: 'cuicas',
        icon: '🪘',
        labelKey: 'scoringSettings.catCuicas',
        fallback: 'Cuicas',
        notes: [78, 79]
      },
      {
        key: 'triangles',
        icon: '🔺',
        labelKey: 'scoringSettings.catTriangles',
        fallback: 'Triangles',
        notes: [80, 81]
      }
    ];
      ...
      return [
        {
          key: 'minimal',
          icon: '🎵',
          label: 'scoringSettings.presetMinimal',
          desc: 'scoringSettings.presetMinimalDesc',
          weights: {
            noteRange: 55,
            programMatch: 10,
            instrumentType: 15,
...

        {
          key: 'balanced',
          icon: '⚖️',
          label: 'scoringSettings.presetBalanced',
          desc: 'scoringSettings.presetBalancedDesc',
          weights: {
            noteRange: 40,
            programMatch: 22,
            instrumentType: 20,
...

        {
          key: 'orchestral',
          icon: '🎻',
          label: 'scoringSettings.presetOrchestral',
          desc: 'scoringSettings.presetOrchestralDesc',
          weights: {
            noteRange: 30,
            programMatch: 28,
            instrumentType: 28,
    ...
    _ensureDefaults() {
      ...
      if (!this.overrides.scoreThresholds)
        this.overrides.scoreThresholds = { ...d.scoreThresholds };
      ...
      if (this.overrides.percussion.drumChannelNonDrumPenalty === undefined) {
        this.overrides.percussion.drumChannelNonDrumPenalty =
          d.percussion.drumChannelNonDrumPenalty;
      }
      if (this.overrides.percussion.nonDrumChannelDrumPenalty === undefined) {
        this.overrides.percussion.nonDrumChannelDrumPenalty =
          d.percussion.nonDrumChannelDrumPenalty;
      }
    ...
    _renderRoutingSection() {
      ...
      const sharedPenalty =
        routing.sharedInstrumentPenalty !== undefined ? routing.sharedInstrumentPenalty : 10;
      return `
      ...
      for (const cat of cats) {
        ...
        for (const noteNum of cat.notes) {
          ...
          for (let i = 0; i < noteSubs.length; i++) {
            ...
            const levelClass =
              i === 0 ? 'ss-chain-close' : i === 1 ? 'ss-chain-similar' : 'ss-chain-distant';
    ...
    _renderPercussionTab() {
      ...
      const dwSum =
        dw.noteRange + dw.instrumentType + dw.polyphony + dw.programMatch + dw.ccSupport;
    ...
    _setDrumDepth(catKey, depth) {
      ...
      dialog
        .querySelectorAll(
          `[data-cat="${catKey}"] .ss-chain-node, .ss-drum-cat-header[data-cat="${catKey}"] .ss-chain-node`
        )
        .forEach((node) => {
    ...
    _onLinkedWeightChange(changedKey, newValue, weightGroup, idPrefix, totalId) {
      ...
      if (!dialog) return;
      for (const k of keys) {
        const sl = dialog.querySelector(
          `.ss-slider[data-key="${k}"]${weightGroup === 'drumWeights' ? '.ss-drum-linked' : '.ss-linked'}`
        );
    ...
    _detectActivePreset() {
      ...
      for (const preset of presets) {
        ...
        if (matches && this.overrides.percussion) {
          if (
            this.overrides.percussion.drumChannelDrumBonus !==
            preset.percussion.drumChannelDrumBonus
          )
            matches = false;
          if (
            matches &&
            this.overrides.percussion.drumChannelNonDrumPenalty !==
              preset.percussion.drumChannelNonDrumPenalty
          )
            matches = false;
          if (
            matches &&
            this.overrides.percussion.nonDrumChannelDrumPenalty !==
              preset.percussion.nonDrumChannelDrumPenalty
          )
            matches = false;
          if (matches && this.overrides.percussion.drumChannelWeights) {
            for (const [k, v] of Object.entries(preset.percussion.drumChannelWeights)) {
              if (this.overrides.percussion.drumChannelWeights[k] !== v) {
                matches = false;
                break;
              }
            }
    ...
    _updatePresetIndicator() {
      ...
      const isModified =
        this.presetSnapshot && JSON.stringify(this.overrides) !== this.presetSnapshot;
