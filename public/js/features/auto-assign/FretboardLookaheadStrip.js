  ...
  class FretboardLookaheadStrip {
    constructor(canvas, opts = {}) {
      ...
      this.windowSeconds = Math.max(
        2,
        Math.min(8, Number.isFinite(opts.windowSeconds) ? opts.windowSeconds : 4)
      );
    ...
    draw() {
      ...
      for (let i = 0; i < visible.length - 1; i++) {
        ...
        const aReleaseSec = Math.max(
          aSec,
          Math.min(b.tick / tps, (Number.isFinite(a.releaseTick) ? a.releaseTick : a.tick) / tps)
        );
        ...
        if (yARelease < yA - 0.5) {
          ...
          ctx.fillRect(colA.x0, yARelease, colA.x1 - colA.x0, yA - yARelease);
          ctx.strokeStyle = HOLD_STROKE;
          ctx.strokeRect(colA.x0 + 0.5, yARelease + 0.5, colA.x1 - colA.x0 - 1, yA - yARelease - 1);
        }
      ...
      if (yEnd < yLast - 0.5) {
        ctx.fillStyle = HOLD_FILL;
        ctx.fillRect(colLast.x0, yEnd, colLast.x1 - colLast.x0, yLast - yEnd);
        ctx.strokeStyle = HOLD_STROKE;
        ctx.strokeRect(colLast.x0 + 0.5, yEnd + 0.5, colLast.x1 - colLast.x0 - 1, yLast - yEnd - 1);
      }
