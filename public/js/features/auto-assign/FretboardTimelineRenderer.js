...
(function () {
  ...
  class FretboardTimelineRenderer {
    constructor(canvas, opts = {}) {
      ...
      this.ticksPerSec =
        Number.isFinite(opts.ticksPerSec) && opts.ticksPerSec > 0 ? opts.ticksPerSec : 480;
      this.totalSec = Number.isFinite(opts.totalSec) && opts.totalSec > 0 ? opts.totalSec : 0;
      ...
      this.pxPerSec =
        Number.isFinite(opts.pxPerSec) && opts.pxPerSec > 0 ? opts.pxPerSec : DEFAULT_PX_PER_SEC;
      this.scrollSec = 0;
      this.playheadSec = 0;
      ...
      this.onViewportChange =
        typeof opts.onViewportChange === 'function' ? opts.onViewportChange : null;
      this._noteHits = [];
    ...
    setTimeline(events) {
      ...
      for (const ch of this._chords) {
        ch._unplayableNotes = Array.isArray(ch.unplayable)
          ? new Set(ch.unplayable.filter((u) => Number.isFinite(u.note)).map((u) => u.note))
          : null;
      }
      this._scheduleDraw();
    }
    ...
    setPxPerSec(px) {
      const next = Math.max(
        MIN_PX_PER_SEC,
        Math.min(MAX_PX_PER_SEC, Number.isFinite(px) ? px : this.pxPerSec)
      );
    ...
    _handWindowX(anchor) {
      ...
      const x1 = rightMm > lastFretMm ? this._fretX(this.numFrets) : this._xFromMm(rightMm);
      return { x0, x1 };
    }
    ...
    destroy() {
      ...
      if (this.canvas?.removeEventListener) {
        if (this._wheelHandler)
          this.canvas.removeEventListener('wheel', this._wheelHandler, this._wheelOpts);
