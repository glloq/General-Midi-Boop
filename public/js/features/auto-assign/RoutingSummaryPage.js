...
(function () {
  ...
  class RoutingSummaryPage {
    ...
    _saveScoringOverrides() {
      try {
        ...
        console.warn('[RoutingSummary] Failed to save scoring config:', e);
      }
    }
    ...
    async show(fileId, filename, channels, onApply) {
      ...
      try {
        ...
        const savedRoutingsP = this.apiClient.getSavedRoutings(fileId).catch(() => null);
        const fileReadP = this.apiClient.readFile(fileId).catch((e) => {
          console.warn('[RoutingSummary] Could not load MIDI data for preview:', e.message);
          return null;
        });
        ...
        try {
          ...
          /* ignore — fall back to auto-selection */
        }
        ...
        if (savedRoutings.length > 0) {
          ...
          for (const r of savedRoutings) {
            ...
            const inst = (this.allInstruments || []).find(
              (i) =>
                i.device_id === r.device_id &&
                (i.custom_name === r.instrument_name || i.name === r.instrument_name)
            );
            if (inst) {
        ...
        for (const [ch, assignment] of Object.entries(this.selectedAssignments)) {
          ...
          const matched =
            options.find((o) => o.instrument.id === assignment.instrumentId) ||
          ...
          if (inst) {
            ...
            const fullInst = (this.allInstruments || []).find(
        ...
        for (const ch of channelKeys) {
          ...
          if (this.autoAdaptation) {
            ...
            const instPoly =
              this._getInstrumentPolyphony(parseInt(ch)) ||
        ...
        try {
          ...
          if (fileResponse?.midiData) {
            ...
            this.midiData =
              raw.midi && raw.midi.tracks
                ? {
                    ...raw.midi,
                    tempo: raw.tempo || raw.midi.tempo,
    ...
    _renderContent(hint = 'all') {
      ...
      try {
        ...
        if (hint === 'all') {
          ...
          if (container)
            container.classList.toggle('rs-with-detail', this.selectedChannel !== null);
    ...
    _updateHeaderState() {
      ...
      if (chBtn) {
        ...
        const iconHTML = iconSpan
          ? iconSpan.outerHTML
          : '<span class="rs-prev-icon">&#9654;</span>';
     ...
     * Falls back to looking up the instrument in allInstruments if
     * seg.gmProgram is missing.
     */
    ...
    _renderSummaryTable(channelKeys) {
      return renderSummaryTable({
        ...
        autoAdaptation: this.autoAdaptation,
        getDisplayName: (inst) => this._getInstrumentDisplayName(inst),
        buildInstrumentOptions: (ch, assignment, isSkipped) =>
          this._buildInstrumentOptions(ch, assignment, isSkipped),
    ...
    _getHandFeasibilityBadge(channel) {
      ...
      let worst = 'unknown';
      const promote = (level) => {
        if ((order[level] || 0) > (order[worst] || 0)) worst = level;
      };
    ...
    _safeRenderDetailPanel(channel) {
      try {
        ...
        console.error(
          '[RoutingSummary] Detail panel render failed for channel',
          channel,
          ':',
          error
        );
    ...
    _renderDetailPanel(channel) {
      ...
      const detailDisplayType =
        analysis?.estimatedCategory && analysis.estimatedCategory !== 'unknown'
          ? analysis.estimatedCategory
          : analysis?.estimatedType || '';
      const typeIcon = detailDisplayType ? getTypeIcon(detailDisplayType) : '';
      const score = assignment?.score || 0;
      const assignedName =
        assignment?.instrumentDisplayName ||
        assignment?.customName ||
        getGmProgramName(assignment?.gmProgram) ||
      ...
      const instrumentChipsHTML =
        options.length > 0 || lowOptions.length > 0
      ...
      const rangeBarsHTML =
        !isDrumChannel && (assignment?.noteRangeMin != null || hasSplitData)
          ? this._renderRangeBars(channel, analysis, assignment)
          : '';

      // Drum note mapping section (for drum channels with assigned instruments)
      const drumMappingHTML =
        isDrumChannel && assignment?.instrumentId ? this._renderDrumMappingSection(channel) : '';

      // Split section — only render if multi-instrument is active (user-accepted) — P2-F.4r
      let splitHTML = '';
      if (isSplit && this.splitAssignments[channel]) {
        const adapt = this.adaptationSettings[ch] || {};
        const splitSemitones =
          this.autoAdaptation && adapt.pitchShift !== 'none'
            ? adapt.transpositionSemitones || 0
            : 0;
      ...
      if (!isSplit && !isSkipped && score > 0 && score < 60 && hasSplitProposal) {
        ...
        const qualityLabel = estimatedQuality
          ? ` (${_t('autoAssign.estimatedScore') || 'score estimé'}: ${estimatedQuality})`
          : '';
        splitSuggestionHTML = `
        ...
        routeHTML =
          ...
          segments
            .map((seg, i) => {
              ...
              const inst = seg.instrumentId
                ? (this.allInstruments || []).find((ii) => ii.id === seg.instrumentId)
                : null;
              const name = inst
                ? this._getInstrumentDisplayName(inst)
                : seg.instrumentName || getGmProgramName(seg.gmProgram) || 'Instrument';
      ...
      if (channelPoly && instPoly) {
        const adapt = this.adaptationSettings[ch] || {};
        const polyActive =
          this.autoAdaptation && adapt.polyReduction && adapt.polyReduction !== 'none';
        const polyOk = polyActive || instPoly >= channelPoly;
        const polyLabel = polyActive
          ? `${channelPoly}\u2192${adapt.polyTarget || instPoly}`
          : `${channelPoly}/${instPoly}`;
    ...
    _renderHandsPreviewSection(channel) {
      ...
      const assignment = this.selectedAssignments?.[String(channel)];
      if (!assignment) return '';
      const instrumentRecord = (this.allInstruments || []).find(
        (i) => i.id === assignment.instrumentId
      );
    ...
    async _mountHandsPreview(channel) {
      ...
      const ticksPerBeat =
        this.midiData?.header?.ticksPerBeat || this.midiData?.ticksPerBeat || 480;
      ...
      if (isFretted && instrumentRecord.device_id != null && instrumentRecord.channel != null) {
        ...
        try {
          ...
          /* fall back to defaults; preview may stay blank */
        }
      }
      ...
      const transpositionSemitones =
        this.autoAdaptation && adapt.pitchShift !== 'none' ? adapt.transpositionSemitones || 0 : 0;
    ...
    _extractInitialOverrides(assignment) {
      return assignment?.handPositionOverrides || assignment?.hand_position_overrides || null;
    }
    ...
    _renderAdaptationBlock(channel) {
      ...
      return renderAdaptationBlock({
        ...
        playableWithTranspose:
          adaptation.pitchShift === 'manual' ? this._computePlayableNotes(ch) : null,
        polyReductionHTML: this._renderPolyReductionSection(
          channel,
          adaptation,
          analysis,
          assignment
    ...
    _renderDrumMappingSection(channel) {
      ...
      if (assignment?.selectedNotes) {
        try {
          instrumentNotes =
            typeof assignment.selectedNotes === 'string'
              ? JSON.parse(assignment.selectedNotes)
              : assignment.selectedNotes;
        } catch (e) {
      ...
      if (!instrumentNotes && assignment) {
        ...
        if (inst?.selected_notes) {
          try {
            instrumentNotes =
              typeof inst.selected_notes === 'string'
                ? JSON.parse(inst.selected_notes)
                : inst.selected_notes;
          } catch (e) {
            instrumentNotes = null;
          }
    ...
    _bindGlobalEvents(channelKeys) {
      ...
      if (settingsBtn) {
        settingsBtn.addEventListener('click', () => {
          ...
          const settingsModal = new window.ScoringSettingsModal(
      ...
      panel.addEventListener(
        ...
        (e) => {
          ...
          if (
            row &&
            !target.closest(
              '.rs-btn-skip, .rs-btn-unskip, .rs-instrument-select, .rs-select-zone, .rs-volume-zone, .rs-volume-slider'
            )
          ) {
        ...
        (e) => {
          ...
          if (transposeBtn) {
            ...
            try {
              ...
              if (typeof window !== 'undefined' && typeof window.showToast === 'function') {
                window.showToast(
                  _t('autoAssign.transposeError', 'Erreur de transposition'),
                  'error'
                );
              }
            }
          ...
          if (polyBtn) {
            ...
            if (ch && !isNaN(delta)) {
              if (!this.adaptationSettings[ch]) this.adaptationSettings[ch] = {};
              const current =
                this.adaptationSettings[ch].polyTarget ||
                this._getInstrumentPolyphony(ch) ||
                getGmDefaultPolyphony(this.selectedAssignments[ch]?.gmProgram) ||
                8;
          ...
          if (removeSeg) {
            this._removeSplitSegment(
              parseInt(removeSeg.dataset.channel),
              parseInt(removeSeg.dataset.seg),
              channelKeys
            );
            return;
          }
          ...
          if (overlapBtn) {
            this._resolveOverlap(
              parseInt(overlapBtn.dataset.channel),
              parseInt(overlapBtn.dataset.overlap),
              overlapBtn.dataset.strategy,
              channelKeys
            );
            return;
          }
          ...
          if (segToggle) {
            ...
            if (muted.has(segIdx)) {
              ...
              if (Object.keys(this.ccSegmentMute[channel]).length === 0)
                delete this.ccSegmentMute[channel];
            } else {
              muted.add(segIdx);
            }
            this._refreshUI(channelKeys, 'detail');
      ...
      panel.addEventListener(
        ...
        (e) => {
          ...
          if (target.matches('.rs-seg-instrument-select')) {
            this._updateSegmentInstrument(
              parseInt(target.dataset.channel),
              parseInt(target.dataset.seg),
              target.value,
              target.dataset.mode,
              channelKeys
            );
          ...
          if (target.matches('.rs-seg-range-input')) {
            this._updateSegmentRange(
              parseInt(target.dataset.channel),
              parseInt(target.dataset.seg),
              target.dataset.bound,
              target.value,
              channelKeys
            );
            return;
      ...
      panel.addEventListener(
        ...
        (e) => {
          ...
          const onMove = (moveE) => {
            ...
            noteValue = Math.max(
              Math.max(physMin, chMin),
              Math.min(Math.min(physMax, chMax), noteValue)
            );
    ...
    _updateSegmentInstrument(channel, segIdx, instrumentId, mode, channelKeys) {
      ...
      for (const opt of [
        ...
        if (opt.instrument.id === instrumentId) {
          inst = opt.instrument;
          break;
        }
      }
      ...
      const semi =
        this.autoAdaptation && adaptSettings.pitchShift !== 'none'
          ? adaptSettings.transpositionSemitones || 0
          : 0;
      const tCh = safeNoteRange(
        (analysis?.noteRange?.min ?? 0) + semi,
        (analysis?.noteRange?.max ?? 127) + semi
      );
    ...
    _addSplitSegment(channel, channelKeys) {
      ...
      const candidates = this._getCompatibleInstrumentsForSegment(
        ch,
        analysis?.noteRange || { min: 0, max: 127 }
      );
      ...
      const sorted = [...data.segments].sort(
        (a, b) => (a.noteRange?.min ?? 0) - (b.noteRange?.min ?? 0)
      );
      ...
      const rangeMin =
        bestGap.size > 0 ? bestGap.min : Math.max(tCh.min, newInst.note_range_min ?? 0);
      const rangeMax =
        bestGap.size > 0 ? bestGap.max : Math.min(tCh.max, newInst.note_range_max ?? 127);
    ...
    _selectInstrument(ch, instrumentId, channelKeys) {
      ...
      this.selectedAssignments[ch] = {
        ...
        supportedCcs:
          selected.instrument.supported_ccs ||
      ...
      const secondSeg = secondInst
        ? {
            ...
            noteRange: safeNoteRange(
              Math.max(secondInst.note_range_min ?? 0, tCh.min),
              Math.min(secondInst.note_range_max ?? 127, tCh.max)
            ),
            fullRange: safeNoteRange(
              secondInst.note_range_min ?? 0,
              secondInst.note_range_max ?? 127
            )
          }
    ...
    _refreshAfterTransposition(channel) {
      ...
      if (panel) {
        ...
        if (rangeHost) {
          const analysis =
            this.channelAnalyses[channel] || this.selectedAssignments[ch]?.channelAnalysis;
          const assignment = this.selectedAssignments[ch];
          const isDrumChannel = channel === 9 || analysis?.estimatedType === 'drums';
          if (
            !isDrumChannel &&
            (assignment?.noteRangeMin != null || this.splitChannels.has(channel))
          ) {
      ...
      if (
        ...
        const newSemi =
          this.autoAdaptation && adapt.pitchShift !== 'none'
            ...
            : 0;
        try {
          this._handsPreviewPanel.setTransposition(newSemi);
        } catch (err) {
          console.warn('[RoutingSummary] hands preview retransposition failed:', err);
        }
      }
    ...
    async _applyRouting() {
      ...
      const { hasTransposition, needsFileModification } = builder.computeModificationFlags(
        assignments,
        hasSplit
      );
      ...
      try {
        ...
        const result = await (window.withProgressToast
          ? window.withProgressToast(
              applyP,
              _t('autoAssign.applying') || _t('autoAssign.analyzing'),
              { thresholdMs: 500 }
            )
          : applyP);
        ...
        for (const [ch, assignment] of Object.entries(assignments)) {
          if (assignment.split) {
            ...
            routing[ch] =
              firstSeg.instrumentChannel !== undefined
    ...
    // Preview / minimap surface lives in RoutingSummaryPreviewControls.js and
    // is mixed onto the prototype below — kept out of this file to keep it
    // under control (AUDIT 2026-05-10 §32).
  ...
  // Audio-preview + minimap methods live in their own mixin file to keep this
  // class manageable (AUDIT 2026-05-10 §32). The mixin script must load BEFORE
  // this one — see public/index.html.
  if (window.RoutingSummaryPreviewControls) {
    Object.assign(RoutingSummaryPage.prototype, window.RoutingSummaryPreviewControls);
  } else {
    console.error(
      '[RoutingSummaryPage] RoutingSummaryPreviewControls mixin missing — preview/minimap methods will not be available. Check script load order in index.html.'
    );
  }
