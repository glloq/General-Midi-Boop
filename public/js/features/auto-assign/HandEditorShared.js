...
(function () {
   ...
   *  null, reset button, etc). Derived from DEFAULT_OVERRIDES so
   *  the schema lives in exactly one place. */
  function emptyOverrides() {
    return cloneOverrides(DEFAULT_OVERRIDES);
  }
  ...
  class HistoryManager {
    ...
    constructor(initial, opts = {}) {
      this._maxHistory =
        Number.isFinite(opts.maxHistory) && opts.maxHistory > 0 ? opts.maxHistory : 50;
    ...
    current() {
      return cloneOverrides(this._history[this._index]);
    }
  ...
  function showUnsavedChangesConfirm(opts = {}) {
    return new Promise((resolve) => {
      ...
      const title = _t(
        opts.titleKey || 'handEditorShared.confirmDiscardTitle',
        opts.titleFallback || 'Modifications non enregistrées'
      );
      const message = _t(
        opts.messageKey || 'handEditorShared.confirmDiscard',
        opts.messageFallback || 'Voulez-vous quitter sans sauvegarder ?'
      );
      const confirm = _t(
        opts.confirmKey || 'handEditorShared.discardConfirmBtn',
