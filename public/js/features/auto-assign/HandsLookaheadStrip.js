...
(function () {
  ...
  class HandsLookaheadStrip {
    constructor(canvas, opts = {}) {
      ...
      this.notes = Array.isArray(opts.notes)
        ? opts.notes.slice().sort((a, b) => a.tick - b.tick)
        : [];
      this.ticksPerSecond =
        Number.isFinite(opts.ticksPerSecond) && opts.ticksPerSecond > 0 ? opts.ticksPerSecond : 480;
      ...
      for (const tr of this.handTrajectories) {
        ...
        for (let i = 0; i + 1 < series.length; i++) {
          ...
          if (env.releaseA > a.sec + 1e-9) {
            ...
            ctx.fillRect(colA.x, yRel, colAR.x + colAR.width - colA.x, yA - yRel);
          }
          if (!env.isInfeasible && a.anchor !== b.anchor && env.arrivalSec < b.sec - 1e-9) {
            ...
            ctx.fillRect(colB.x, yB, colBR.x + colBR.width - colB.x, yArr - yB);
          }
        }
