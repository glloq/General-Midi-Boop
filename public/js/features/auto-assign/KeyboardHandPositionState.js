...
(function () {
  ...
  class KeyboardHandPositionState {
    ...
    constructor(opts = {}) {
      ...
      this.lerpHalfLifeMs =
        Number.isFinite(opts.lerpHalfLifeMs) && opts.lerpHalfLifeMs > 0 ? opts.lerpHalfLifeMs : 85;
    ...
    _displaySpanFor(hand, anchor) {
      const numFingers =
        Number.isFinite(hand.numFingers) && hand.numFingers > 0
      ...
      if (this.layout === 'piano') {
        ...
        if ((numFingers & 1) === 1) {
          ...
          highMidi =
            Number.isFinite(wLo) && Number.isFinite(wHi) && wHi - wLo === 2 ? wLo + 1 : wHi;
        }
      ...
      for (const ev of timeline || []) {
        if (ev.type === 'shift' && ev.handId && Number.isFinite(ev.toAnchor)) {
          ...
          if (ev.motion && Number.isFinite(ev.motion.requiredSec) && ev.motion.requiredSec > 0) {
            ...
            ev.motion &&
            Number.isFinite(ev.motion.availableSec) &&
    ...
    _handIdxById(id) {
      return this.hands.findIndex((h) => h.id === id);
    }
    ...
    _buildHands() {
      const cfg = _parseHandsCfg(this.instrument);
      const hands = (cfg && cfg.hands ? cfg.hands : []).map((h, i) => {
        let span;
        ...
        const numFingers =
          Number.isFinite(h.num_fingers) && h.num_fingers > 0 ? h.num_fingers : span + 1;
        const id = h.id || `h${i + 1}`;
        return {
          id,
          span,
    ...
    _initialAnchorFor(hand, i, total) {
      ...
      const seed =
        ext.lo + Math.round(((i + 0.5) / Math.max(1, total)) * (ext.hi - ext.lo - hand.span));
