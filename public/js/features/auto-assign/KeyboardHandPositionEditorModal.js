...
(function () {
  ...
  function _kbIsBlackKey(m) {
    return MidiConstants.isBlackKey(m);
  }
  ...
  class KeyboardHandPositionEditorModal extends window.BaseModal {
    ...
    get _displayedAnchor() {
      return this.state._displayedAnchors;
    }
    get _handAnchorTimeline() {
      return this.state.simulationTimeline;
    }
    get _problems() {
      return this.state.problems;
    }
    get _unplayableSet() {
      return this.state.unplayableSet;
    }
    ...
    _runFeasibility() {
      ...
      try {
        timeline =
          Feas.simulateHandWindows(this.notes, this.instrument, {
    ...
    _ensureAnimLoop() {
      ...
      const tick = (now) => {
        ...
        const dt =
          this._lastAnimTime != null ? Math.min(0.1, (now - this._lastAnimTime) / 1000) : 0;
    ...
    _jumpToProblem(direction) {
      ...
      if (direction > 0) {
        ...
        for (let i = list.length - 1; i >= 0; i--) {
          if (list[i].sec < this._currentSec - EPS) {
            idx = i;
            break;
          }
        }
    ...
    _mountRollRenderer() {
      ...
      this.rollRenderer = new window.KeyboardRollRenderer(this.rollCanvas, this.rollHost, {
        ticksPerSec: this.ticksPerSec,
        totalSec: this._totalSec,
    ...
    _openNotePopover(hit, evt) {
      ...
      popover.style.cssText =
        `position:fixed;left:${(evt.clientX || 0) + 8}px;top:${(evt.clientY || 0) + 8}px;` +
      ...
      popover.innerHTML = `
                <div style="margin-bottom:6px;font-weight:600;">${_t(
                  'keyboardHandEditor.pickHand',
                  'Affecter à la main :'
                )}</div>
    ...
    async _play() {
      ...
      try {
        ...
        this._setStatus(
          `${_t('keyboardHandEditor.playFailed', 'Lecture impossible')}: ${err.message || err}`
        );
      }
    }
    ...
    _toggleMute(btn) {
      ...
      if (isMuted) {
        ...
        this._mutedBeforePlay = synth.mutedChannels ? Array.from(synth.mutedChannels) : [];
    ...
    async _save() {
      ...
      try {
        ...
        this._setStatus(
          `${_t('keyboardHandEditor.saveFailed', 'Sauvegarde impossible')}: ${err.message || err}`
        );
      }
    }
