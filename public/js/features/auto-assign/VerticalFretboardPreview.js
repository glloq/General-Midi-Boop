...
(function () {
  ...
  class VerticalFretboardPreview {
    constructor(canvas, opts = {}) {
      ...
      this.maxFingers =
        Number.isFinite(opts.maxFingers) && opts.maxFingers > 0 ? opts.maxFingers : 4;
    ...
    _drawStringSlidingFingerRanges(x0, x1) {
      ...
      for (let s = 1; s <= numF; s++) {
        ...
        const isAnchored =
          sustainingAnchored.has(s) || (anchoredFallback && anchoredFallback.has(s));
    ...
    _drawActivePositions() {
      ...
      for (const p of this.activePositions) {
        ...
        const x =
          p.fret === 0 ? this._fretX(0) - 8 : (this._fretX(p.fret - 1) + this._fretX(p.fret)) / 2;
        ctx.fillStyle = 'rgba(37, 99, 235, 0.85)';
        ctx.beginPath();
