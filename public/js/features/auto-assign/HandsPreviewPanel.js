  ...
  class HandsPreviewPanel {
    ...
    setTransposition(semitones) {
      ...
      if (this.lookahead && typeof this.lookahead.setNotes === 'function') {
        try {
          this.lookahead.setNotes(this.notes);
        } catch (_) {
          /* keep going */
        }
      }
    ...
    _render() {
      ...
      header.style.cssText =
        'display:flex;gap:8px;align-items:center;padding:6px 8px;border-bottom:1px solid #e5e7eb;cursor:pointer;user-select:none;';
      ...
      const editorClassName =
        this.mode === 'frets'
          ...
          : this.mode === 'semitones'
            ? 'KeyboardHandPositionEditorModal'
            : null;
      const showEditorBtn =
      ...
      const editorBtnHtml = showEditorBtn
        ? `<button class="hpp-open-editor" type="button"
                          ...
                          title="${_t(
                            'handPositionEditor.openButtonTitle',
      ...
      if (on) {
        if (!spinner) {
          ...
          spinner.style.cssText =
            'position:absolute;inset:0;display:flex;align-items:center;justify-content:center;background:rgba(255,255,255,0.7);font-size:12px;color:#374151;gap:8px;z-index:10;';
    ...
    _renderKeyboardLayout(body) {
      ...
      lookCanvas.style.cssText =
        'width:100%;height:140px;display:block;border:1px solid #e5e7eb;border-bottom:none;border-radius:4px 4px 0 0;';
      ...
      kbCanvas.style.cssText =
        'width:100%;height:120px;display:block;border:1px solid #e5e7eb;border-radius:0 0 4px 4px;';
      body.appendChild(kbCanvas);

      const rangeMin = Number.isFinite(this.instrument?.note_range_min)
        ? this.instrument.note_range_min
        : 21;
      const rangeMax = Number.isFinite(this.instrument?.note_range_max)
        ? this.instrument.note_range_max
        : 108;
    ...
    _renderFretsLayout(body) {
      ...
      if (typeof window !== 'undefined' && window.FretboardLookaheadStrip) {
        ...
        strip.style.cssText =
          'width:100%;height:140px;display:block;border:1px solid #e5e7eb;border-bottom:none;border-radius:4px 4px 0 0;background:#f5f7fb;';
      ...
      const PreviewClass =
        typeof window !== 'undefined' && window.FretboardHandPreview
      ...
      const frettingId =
        (handsArr.find((h) => h && h.id === 'fretting') || handsArr[0])?.id || 'fretting';
    ...
    _engineCacheKey() {
      ...
      let ov = '';
      try {
        ov = this.overrides ? JSON.stringify(this.overrides) : '';
      } catch (_) {
        ov = '';
      }
    ...
    _wireEngine() {
      ...
      this.engine.on('chord', (e) => {
        ...
        if (this.keyboard) {
          ...
          this.keyboard.setUnplayableNotes(
            unplayable.map((u) => ({ note: u.note, hand: u.handId }))
          );
        }
        if (this.lookahead) {
        ...
        if (this.fretboard) {
          ...
          this.fretboard.setUnplayablePositions(
            unplayable.filter((u) => Number.isFinite(u.string) && Number.isFinite(u.fret))
          );
      ...
      this.engine.on('tick', (e) => {
        ...
        if (this.fretboard && typeof this.fretboard.setCurrentTime === 'function') {
    ...
    _refreshFretboardTrajectory() {
      ...
      if (typeof this.engine.getHandTrajectories !== 'function') return;
      const tps =
        Number.isFinite(this.ticksPerBeat) &&
        this.ticksPerBeat > 0 &&
        Number.isFinite(this.bpm) &&
        this.bpm > 0
          ? this.ticksPerBeat * (this.bpm / 60)
          : null;
      const handsArr = _hands(this.instrument);
      const fretting = handsArr.find((h) => h && h.id === 'fretting') || handsArr[0];
      const map = this.engine.getHandTrajectories();
    ...
    toggleDisabledNote(midi) {
      ...
      const idx = this.overrides.disabled_notes.findIndex(
        (n) => n.tick === tick && n.note === midi
      );
      if (idx >= 0) {
