...
(function () {
  ...
  class KeyboardMinimapRenderer {
    ...
    constructor(canvas, host, opts = {}) {
      ...
      this.getDisplayedAnchor =
        typeof opts.getDisplayedAnchor === 'function' ? opts.getDisplayedAnchor : null;
    ...
    _drawTrajectoryBands(ctx, samples, hand, xPerSec, yOfPitch, fill) {
      ...
      for (let i = 0; i < samples.length - 1; i++) {
        ...
        if (
          Number.isFinite(next.fromSec) &&
          Number.isFinite(next.fromAnchor) &&
    ...
    _samplesFor(hand) {
      ...
      if (Array.isArray(series) && series.length > 0) {
        ...
        const seedAnchor = Number.isFinite(first.fromAnchor) ? first.fromAnchor : first.anchor;
    ...
    _overrideOnlySamplesFor(hand) {
      const list = this._overrideAnchors
        .filter(
          (a) => a && a.handId === hand.id && Number.isFinite(a.tick) && Number.isFinite(a.anchor)
        )
