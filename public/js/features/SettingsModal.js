...
class SettingsModal {
  // Toggle buttons whose visibility is driven by a boolean setting.
  // settingKey → DOM element id + the settings:* event emitted on change.
  static TOGGLE_BUTTONS = [
    {
      settingKey: 'showDebugButton',
      elementId: 'debugToggle',
      event: 'settings:debug_button_changed'
    },
    {
      settingKey: 'showCalibrationButton',
      elementId: 'calibrationBtn',
      event: 'settings:calibration_button_changed'
    },
    {
      settingKey: 'showLightingButton',
      elementId: 'lightingBtn',
      event: 'settings:lighting_button_changed'
    },
    {
      settingKey: 'showPlaylistButton',
      elementId: 'playlistBtn',
      event: 'settings:playlist_button_changed'
    },
    {
      settingKey: 'showKeyboardButton',
      elementId: 'openKeyboardBtn',
      event: 'settings:keyboard_button_changed'
    },
    {
      settingKey: 'showInstrumentsButton',
      elementId: 'instrumentsBtn',
      event: 'settings:instruments_button_changed'
    },
    {
    ...
    try {
      ...
      if (saved) {
        ...
        if (parsed.keyboardKeys !== undefined && parsed.keyboardOctaves === undefined) {
          ...
          this.logger?.info(
            `Migrated keyboardKeys (${oldKeys}) to keyboardOctaves (${parsed.keyboardOctaves})`
          );
        }
    ...
    if (soundBankSelect && soundBankDesc) {
      soundBankSelect.addEventListener('change', (e) => {
        ...
        if (selected && selected.descKey) {
          soundBankDesc.textContent =
            i18n.t(selected.descKey) || i18n.t('settings.soundBank.description') || '';
        } else {
  ...
  open() {
    ...
    if (calibrationButtonToggle)
      calibrationButtonToggle.checked = this.settings.showCalibrationButton;
    ...
    if (instrumentsButtonToggle)
      instrumentsButtonToggle.checked = this.settings.showInstrumentsButton;
    ...
    const serialPortsSection = this.modal.querySelector('#serialMidiPortsSection');
    if (serialPortsSection)
      serialPortsSection.style.display = this.settings.serialMidiEnabled ? 'block' : 'none';
    ...
    const startupUpdateCheckToggle = this.modal.querySelector('#startupUpdateCheckToggle');
    if (startupUpdateCheckToggle)
      startupUpdateCheckToggle.checked = this.settings.startupUpdateCheck;
    const startupBetaNotifToggle = this.modal.querySelector('#startupBetaNotifToggle');
  ...
  save() {
    ...
    try {
      ...
      if (saved) {
        const parsed = JSON.parse(saved);
        if (typeof parsed.virtualInstrument === 'boolean')
          currentVirtual = parsed.virtualInstrument;
      }
    } catch (e) {
      /* ignore */
    }
    ...
    const newSettings = {
      ...
      showDebugButton: debugButtonToggle
        ? debugButtonToggle.checked
        : this.settings.showDebugButton,
      showCalibrationButton: calibrationButtonToggle
        ? calibrationButtonToggle.checked
        : this.settings.showCalibrationButton,
      showLightingButton: lightingButtonToggle
        ? lightingButtonToggle.checked
        : this.settings.showLightingButton,
      showPlaylistButton: playlistButtonToggle
        ? playlistButtonToggle.checked
        : this.settings.showPlaylistButton,
      showKeyboardButton: keyboardButtonToggle
        ? keyboardButtonToggle.checked
        : this.settings.showKeyboardButton,
      keyboardLayout: keyboardLayoutSelect
        ? keyboardLayoutSelect.value
        : this.settings.keyboardLayout || 'azerty',
      showInstrumentsButton: instrumentsButtonToggle
        ? instrumentsButtonToggle.checked
        : this.settings.showInstrumentsButton,
      ...
      serialMidiEnabled: serialMidiToggle
        ...
        : this.settings.serialMidiEnabled,
      showLoadingAnimation: loadingAnimationToggle
        ? loadingAnimationToggle.checked
        : this.settings.showLoadingAnimation,
      soundBank: soundBankSelect ? soundBankSelect.value : this.settings.soundBank,
      startupUpdateCheck: startupUpdateCheckToggle
        ? startupUpdateCheckToggle.checked
        : this.settings.startupUpdateCheck,
      startupBetaNotif: startupBetaNotifToggle
        ? startupBetaNotifToggle.checked
        : this.settings.startupBetaNotif
    };
    ...
    const changedToggleButtons = SettingsModal.TOGGLE_BUTTONS.filter(
      (b) => newSettings[b.settingKey] !== this.settings[b.settingKey]
    );
    ...
    if (pianoRollChanged) {
      ...
      this.logger?.info(
        `🎹 ${i18n.t(newSettings.showPianoRoll ? 'settings.pianoRoll.enabled' : 'settings.pianoRoll.disabled')}`
      );
    }
    if (lyricsChanged) {
      this.eventBus?.emit('settings:lyrics_changed', { enabled: newSettings.showLyrics });
      this.logger?.info(
        `🎤 ${i18n.t(newSettings.showLyrics ? 'settings.lyrics.enabled' : 'settings.lyrics.disabled')}`
      );
    }
    for (const b of changedToggleButtons) {
      this.eventBus?.emit(b.event, { enabled: newSettings[b.settingKey] });
      this._applyToggleButton(b.elementId, newSettings[b.settingKey]);
    }
    if (midiClockChanged)
      this.eventBus?.emit('settings:midi_clock_changed', { enabled: newSettings.midiClockEnabled });
    if (serialMidiChanged)
      this.eventBus?.emit('settings:serial_midi_changed', {
        enabled: newSettings.serialMidiEnabled
      });
    if (soundBankChanged) {
      this.eventBus?.emit('settings:sound_bank_changed', { bankId: newSettings.soundBank });
      this.logger?.info(
        `🔊 ${i18n.t('settings.soundBank.changed') || 'Sound bank changed'}: ${newSettings.soundBank}`
      );
    }
  ...
  applySettings() {
    ...
    for (const b of SettingsModal.TOGGLE_BUTTONS) {
      this._applyToggleButton(b.elementId, this.settings[b.settingKey]);
    }
  }

  _applyToggleButton(elementId, show) {
    const el = document.getElementById(elementId);
    if (el) el.style.display = show ? 'flex' : 'none';
  }

  applyLoopCreatorButton(show) {
    this._applyToggleButton('loopCreatorBtn', show);
  }
  applyDebugButton(show) {
    this._applyToggleButton('debugToggle', show);
  }
  applyCalibrationButton(show) {
    this._applyToggleButton('calibrationBtn', show);
  }
  applyLightingButton(show) {
    this._applyToggleButton('lightingBtn', show);
  }
  applyPlaylistButton(show) {
    this._applyToggleButton('playlistBtn', show);
  }
  applyKeyboardButton(show) {
    this._applyToggleButton('openKeyboardBtn', show);
  }
  applyInstrumentsButton(show) {
    this._applyToggleButton('instrumentsBtn', show);
  }
