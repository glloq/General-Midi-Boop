...
class DrumPatternEditor {
  ...
  _setPianoRollVisible(visible) {
    const pianoRollWrapper = this.modal.container
      ?.querySelector('.notes-section')
      ?.querySelector('.piano-roll-wrapper');
    if (pianoRollWrapper) pianoRollWrapper.style.display = visible ? '' : 'none';
  }

  /** Refresh the main editor toolbar (mode / edit / undo-redo buttons). */
  _refreshModalToolbar() {
    const ea = this.modal.editActions;
    ea.updateModeButtons?.();
    ea.updateEditButtons?.();
    ea.updateUndoRedoButtonsState?.();
  }
  ...
  _initGridRenderer() {
    ...
    if (this.modal.pianoRoll) {
      ...
      if (pr.xrange && this.gridCanvasEl.width) {
        this.gridRenderer.ticksPerPixel =
          pr.xrange / (this.gridCanvasEl.width - this.gridRenderer.headerWidth);
      }
      if (pr.xoffset) {
  ...
  _handleGridEditVelocity(e) {
    ...
    this._showVelocityInput(
      index,
      canvasRect.left + x + window.scrollX,
      canvasRect.top + y + window.scrollY
    );
  }
