...
(function () {
  ...
  class TunerModal extends BaseModal {
    ...
    _renderInstrumentRow() {
      ...
      const connectedOpts = this._connectedInstruments
        .map((inst) => {
      ...
      const presetOpts = Object.keys(TUNER_PRESETS)
        .map((presetKey) => {
      ...
      if (connectedOpts) {
        groups.push(
          `<optgroup label="${this.escape(this.t('tuner.connectedInstruments'))}">${connectedOpts}</optgroup>`
        );
      }
      groups.push(
        `<optgroup label="${this.escape(this.t('tuner.genericPresets'))}">${presetOpts}</optgroup>`
      );
    ...
    _renderInstrumentPicker(sourceKey) {
      ...
      if (sourceKey.startsWith('connected:')) {
        ...
        const inst = this._connectedInstruments.find(
          (x) => x.deviceId === deviceId && x.channel === channel
        );
    ...
    onClose() {
      ...
      if (this.onCloseCb) {
        try {
          ...
          this.logger.warn('TunerModal', 'onCloseCb threw:', e);
        }
      }
    }
    ...
    async _startListening() {
      ...
      try {
        ...
        this._showError(
          `${this.t('tuner.backendError')}: ${err && err.message ? err.message : err}`
        );
    ...
    _handlePitchEvent(payload) {
      ...
      if (ring.length >= 2) {
        ...
        else if (ratio > 0.3 && ratio < 0.36) corrected = freq * 3;
      }
    ...
    _updateDisplay(freq) {
      ...
      const targetMidi = mode === 'auto' ? Math.round(detectedMidiFloat) : this.state.targetMidi;
    ...
    _resetDisplay() {
      ...
      if (noteEl) {
        noteEl.textContent =
          this.state.mode === 'auto' ? '—' : this._formatNote(this.state.targetMidi);
      }
