  ...
  StartupUpdateNotif.run = async function (api) {
    ...
    try {
      ...
      if (stable.majorMinorChanged) {
        ...
        const text =
          tpl && !tpl.includes('.') ? tpl : `New version v${stable.remoteVersion} available!`;
        _showBanner(`🆕 ${text}`, 'linear-gradient(135deg, #16a34a, #15803d)', null, null, 8000);
        ...
        _showBanner(`🔶 ${text}`, 'linear-gradient(135deg, #d97706, #b45309)', null, null, 5000);
      }
    } catch (_e) {
      /* silent — startup check must never crash the app */
    }
  };
