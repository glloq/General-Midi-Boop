...
class InstrumentCapabilitiesModal extends BaseModal {
  ...
  _showInstrument(index) {
    ...
    if (progressEl) {
      progressEl.textContent = _t('instrumentCapabilities.progress', { current: index + 1, total }) || `${index + 1} / ${total}`;
    }
    ...
    switch (field.type) {
      ...
      case 'select':
        if (field.field === 'note_selection_mode') {
          ...
          const types = ['keyboard','synth','drums','bass','guitar','strings','brass','woodwind','pad','sampler','other'];
      ...
      case 'array': {
        ...
        const commonCCs = [
          {v:1,l:'CC1 — Modulation'},{v:7,l:'CC7 — Volume'},{v:10,l:'CC10 — Pan'},
          {v:11,l:'CC11 — Expression'},{v:64,l:'CC64 — Sustain'},{v:71,l:'CC71 — Resonance'},
          {v:72,l:'CC72 — Release'},{v:73,l:'CC73 — Attack'},{v:74,l:'CC74 — Brightness'},
          {v:91,l:'CC91 — Reverb'},{v:93,l:'CC93 — Chorus'}
        ];
  ...
  async _complete() {
    ...
    try {
      ...
      if (response && response.success) {
        ...
        if (nextBtn) { nextBtn.disabled = false; nextBtn.textContent = _t('instrumentCapabilities.complete') || 'Terminer'; }
      }
    } catch (err) {
  ...
  async _applyDefaults() {
    ...
    try {
      ...
      await this._alert(_t('instrumentCapabilities.defaultsFailed') || 'Impossible de charger les valeurs par défaut');
    }
  }
  ...
  async _autoCreateStringInstrument(instrument, type) {
    const presetMap = { guitar: 'guitar_standard', bass: 'bass_4_standard', strings: 'guitar_standard' };
    try {
      const existing = await this.apiClient.sendCommand('string_instrument_get', {
        device_id: instrument.id, channel: 0
      });
      if (existing && existing.instrument) { this._showStringBanner(true); return; }
      ...
      if (presetResponse && presetResponse.preset) {
        ...
        await this.apiClient.sendCommand('string_instrument_create', {
          ...
          num_strings: p.strings, num_frets: p.frets,
          tuning: p.tuning, is_fretless: p.fretless || false, capo_fret: 0
        });
  ...
  _showStringBanner(alreadyExists) {
    ...
    const msg = alreadyExists
      ? (_t('stringInstrument.configExists') || 'Configuration d\'instrument à cordes déjà existante.')
      : (_t('stringInstrument.configCreated') || 'Configuration d\'instrument à cordes créée automatiquement.');
