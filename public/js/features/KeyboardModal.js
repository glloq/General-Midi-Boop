...
class KeyboardModal {
  ...
  updateTranslations() {
    ...
    // Velocity
    const velocityLabel = this.container.querySelector(
      '#velocity-control-panel .velocity-label-vertical'
    );
    if (velocityLabel) velocityLabel.textContent = this.t('keyboard.velocity');

    // Modulation
    const modulationLabel = this.container.querySelector(
      '#modulation-control-panel .velocity-label-vertical'
    );
  ...
  setupEventListeners() {
    ...
    this._eventUnsubs = [
      ...
      this.eventBus.on('bluetooth:disconnected', async (_data) => {
        this.logger.info(
          '[KeyboardModal] Bluetooth device disconnected, refreshing device list...'
        );
        await refresh();
      }),
  ...
  close() {
    ...
    if (this._activeView && typeof this._activeView.unmount === 'function') {
      try {
        ...
        this.logger.warn('[KeyboardModal] view.unmount() in close() failed:', e);
      }
    }
    this._activeView = null;
    this._activeViewKind = null;
  ...
  updatePianoDisplay() {
    if (this.viewMode === 'fretboard') {
      ...
      document.querySelectorAll('.fretboard-container .fret-dot.piano-key').forEach((dot) => {
        const pos =
          dot.dataset.string !== undefined && dot.dataset.fret !== undefined
  ...
  async loadDeviceCapabilities(deviceId, channel) {
    ...
    try {
      ...
      this.logger.info(
        `[KeyboardModal] Capacités chargées pour ${deviceId} ch${channel}:`,
        this.selectedDeviceCapabilities
      );
    } catch (error) {
      this.logger.warn(
        `[KeyboardModal] Impossible de charger les capacités pour ${deviceId}:`,
        error
      );
      this.selectedDeviceCapabilities = null;
    }
  ...
  _activateView(viewKind, options = {}) {
    ...
    const ViewClass =
      registry && typeof registry.get === 'function' ? registry.get(viewKind) : null;
    ...
    if (this._activeView && typeof this._activeView.unmount === 'function') {
      try {
        ...
        this.logger.warn('[KeyboardModal] view.unmount() failed:', e);
      }
    }
  ...
  _legacyRenderForMode(mode) {
    ...
    else if (mode === 'piano-slider' && typeof this.generatePianoSlider === 'function')
      this.generatePianoSlider();
    else if (mode === 'keyboard-list' && typeof this.renderKeyboardList === 'function')
      this.renderKeyboardList();
  ...
  setOctaves(octaves) {
    ...
    this.logger.info(
      `[KeyboardModal] Nombre d'octaves changé: ${this.octaves} (${this.visibleNoteCount} touches)`
    );
  ...
  _maybeSendStringFretCC(keyEl) {
    if (
      !keyEl ||
      !keyEl.dataset ||
      keyEl.dataset.string === undefined ||
      keyEl.dataset.fret === undefined
    ) {
      return;
    }
    ...
    if (this.selectedDevice.isVirtual) {
      this.logger?.info?.(
        `🎸 [Virtual] CC${ccStringNumber}=${stringVal} (string ${stringIdx}) CC${ccFretNumber}=${fretVal} (fret ${fret})`
      );
      return;
    }
  ...
  getInstrumentNoteRange() {
    ...
    const count =
      Number.isFinite(this.visibleNoteCount) && this.visibleNoteCount > 0
        ? this.visibleNoteCount
        : 36;
  ...
  getAccordionConfig() {
    ...
    const bass_system = ['stradella', 'free'].includes(rawBass) ? rawBass : 'stradella';
