    ...
    class LoopManagerOutputRouter {
        ...
        async loadDevices() {
            ...
            try {
                ...
                this.cachedDevices = (allDevices || []).filter(d => d.status === 2 || d.connected === true);
            } catch (err) {
        ...
        refreshUI() {
            ...
            if (label) label.textContent = isDev ? m.t('loopManager.outputLive') : m.t('loopManager.outputSynth');
            if (btn) {
        ...
        panicTarget(target) {
            ...
            this.modal.api.sendCommand('midi_panic', { deviceId: target.deviceId })
