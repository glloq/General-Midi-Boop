...
class MidiSynthesizer {
  ...
  static GM_FAMILY_GAIN = [
    ...
    1.05, // 16-23 Organ
    1.1, // 24-31 Guitar
    1.5, // 32-39 Bass            ← compensates "très faible"
   ...
   */
  /**
   * Show `msg` via HandPositionWarningsToast, falling back to the logger.
   * Returns true if the toast was shown synchronously.
   * @private
   */
  static _showWarningToast(msg) {
  ...
  static _notifyBankFallback(from, to) {
    ...
    if (MidiSynthesizer._showWarningToast(msg)) return;
    // The toast module loads lazily; retry once it's there.
    const started = Date.now();
    const iv = setInterval(() => {
      if (MidiSynthesizer._showWarningToast(msg)) {
        clearInterval(iv);
   ...
   * SettingsSF2.loadCustomBanks after GET /api/sf2. One-shot.
   */
  static notifyDefaultSf2Missing() {
    ...
    const msg =
      'Banque son par défaut absente sur le serveur. Lance `npm run install-default-sf2` ou place un fichier dans assets/sf2/default.sf2 pour activer le son.';
    if (!MidiSynthesizer._showWarningToast(msg)) {
      (window.logger || console).warn('[MidiSynthesizer]', msg);
    }
  }
  ...
  static _notifyCustomSf2Fallback() {
    const msg =
      'Le SF2 personnalisé ne contient pas cet instrument GM — retour à la banque standard pour ce canal.';
    if (!MidiSynthesizer._showWarningToast(msg)) {
      (window.logger || console).warn('[MidiSynthesizer]', msg);
    }
  }
  ...
  constructor() {
    ...
    this.drumMinDurations = {
      ...
      57: 2.0,
      55: 1.5,
      52: 2.0, // Crashes, Splash, China
      51: 1.0,
      59: 1.0,
      53: 0.8, // Rides, Ride Bell
      46: 0.8, // Open Hi-Hat
  ...
  _loadDrumPreset(note, kitProgram = 0, bankId = this.currentBankId) {
    ...
    if (currentKitEntry) {
      addCandidate(
        this._buildDrumPresetEntry(this.currentBankSuffix, currentKitEntry.bankIndex, note)
      );
    }
  ...
  async loadDrumKit() {
    ...
    this.log(
      'info',
      `Loading ${usedNotes.size} drum presets for kit ${kit} (bank: ${this.currentBankId})`
    );
  ...
  /**
   * Collect the distinct (bank, program) pairs used by the loaded sequence
   * (and, when `scanConfiguredChannels`, every configured non-drum channel).
   * Key is `${bankId}|${program}` so a channel with a per-instrument custom
   * SF2 is warmed on its own bank. Always yields at least the default piano
   * on the global bank.
   * @returns {{usedPairs: Map, hasDrums: boolean}}
   * @private
   */
  _collectUsedInstrumentPairs(scanConfiguredChannels) {
    ...
    if (usedPairs.size === 0) {
      usedPairs.set(this._instKey(this.currentBankId, 0), {
        program: 0,
        bankId: this.currentBankId
      });
    }

    return { usedPairs, hasDrums };
  }
  ...
  async preloadInstruments() {
    const { usedPairs, hasDrums } = this._collectUsedInstrumentPairs(true);

    this.log(
      'info',
      `Preloading ${usedPairs.size} instruments + ${hasDrums ? 'drums' : 'no drums'}`
    );
   ...
   */
  _preloadNonBlocking() {
    const { usedPairs, hasDrums } = this._collectUsedInstrumentPairs(false);
    ...
    if (hasDrums && this.drumPresets.size === 0) {
      allLoaded = false;
      this.loadDrumKit().catch((e) => this.log('warn', 'Background drum load failed:', e.message));
    }
    return allLoaded;
  }
  ...
  _preloadCurrentChannelPrograms() {
    ...
    for (let ch = 0; ch < 16; ch++) {
      ...
      try {
        this.loadInstrument(program, bankId);
      } catch (e) {
        /* ignore */
      }
    }
  ...
  setChannelInstrument(channel, program, sf2Id = null) {
    ...
    const normSf2 =
      sf2Id === null || sf2Id === undefined || sf2Id === ''
        ? null
        : typeof sf2Id === 'number'
          ? sf2Id
          : parseInt(sf2Id, 10) || null;
  ...
  loadSequence(sequence, tempo = 120, ticksPerBeat = 480, tempoEvents = null) {
    ...
    this.log(
      'info',
      `Sequence loaded: ${this.sequence.length} notes, ${this.ticksToSeconds(maxEndTick).toFixed(2)}s at ${tempo} BPM${this.tempoMap.length > 0 ? `, ${this.tempoMap.length} tempo changes` : ''}`
    );
  }
  ...
  _regenerateReverbIR(decaySeconds) {
    ...
    if (this.drumReverbNode) {
      try {
        this.drumReverbNode.buffer = buffer;
      } catch (e) {
        /* ignore */
      }
    }
    if (this.melodyReverbNode) {
      try {
        this.melodyReverbNode.buffer = buffer;
      } catch (e) {
        /* ignore */
      }
    }
  ...
  _setupDrumBus() {
    ...
    this.log(
      'info',
      `Audio bus initialized (melody reverbMix=${reverbMix} for bank ${this.currentBankId})`
    );
  }
  ...
  applyBankEffects(effects) {
    ...
    const src = effects || {};
    // Numeric value from `src[key]`, or `dflt` when missing/null.
    const num = (key, dflt) =>
      src[key] !== undefined && src[key] !== null ? Number(src[key]) : dflt;

    const reverbMix = num('reverb_mix', defaultReverbMix);
    const reverbDecay = num('reverb_decay_s', 1.2);
    const echoMix = num('echo_mix', 0.0);
    const echoTimeMs = num('echo_time_ms', 250);
    const echoFeedback = num('echo_feedback', 0.3);
  ...
  playNote(note, velocity, channel, duration, time = null) {
    ...
    try {
      ...
      if (this.echoDelayNode && this.echoWetGain && this.echoWetGain.gain.value > 0.001) {
  ...
  async play() {
    ...
    if (this.isPaused) {
      this.isPaused = false;
      this.startTime =
        this.audioContext.currentTime - this.ticksToSeconds(this.currentTick - this.startTick);
    } else {
      this.currentTick = this.startTick;
  ...
  dispose() {
    ...
    if (this.drumDryGain) {
      try {
        this.drumDryGain.disconnect();
      } catch (e) {}
    }
    if (this.drumReverbGain) {
      try {
        this.drumReverbGain.disconnect();
      } catch (e) {}
    }
    if (this.drumReverbNode) {
      try {
        this.drumReverbNode.disconnect();
      } catch (e) {}
    }
    if (this.melodyDryGain) {
      try {
        this.melodyDryGain.disconnect();
      } catch (e) {}
    }
    if (this.melodyReverbGain) {
      try {
        this.melodyReverbGain.disconnect();
      } catch (e) {}
    }
    if (this.melodyReverbNode) {
      try {
        this.melodyReverbNode.disconnect();
      } catch (e) {}
    }
    if (this.echoDelayNode) {
      try {
