...
class MqttLightDriver extends BaseLightingDriver {
  ...
  async connect() {
    try {
      ...
      this._currentColors = new Array(this.device.led_count)
        .fill(null)
        .map(() => ({ r: 0, g: 0, b: 0 }));

      this.client = mqtt.default
        ? mqtt.default.connect(this.brokerUrl, {
      ...
      this.connected = true;
      this.logger.info(
        `MQTT Light driver connected: ${this.brokerUrl}, firmware=${this.firmware}, topic=${this.baseTopic}`
      );
      this.emit('connected');
    } catch (error) {
  ...
  allOff() {
    ...
    switch (this.firmware) {
      ...
      default:
        this._publish(
          `${this.baseTopic}/set`,
          JSON.stringify({ state: 'OFF', color: { r: 0, g: 0, b: 0 }, brightness: 0 })
        );
    }

    this._currentColors.forEach((c) => {
      c.r = 0;
      c.g = 0;
      c.b = 0;
    });
