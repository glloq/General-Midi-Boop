...
function applyScoringOverrides(overrides) {
  ...
  if (overrides.scoreThresholds) {
    ...
    if (t.acceptable !== undefined)
      ScoringConfig.scoreThresholds.acceptable = Math.max(0, Math.min(100, Number(t.acceptable)));
    if (t.minimum !== undefined)
      ScoringConfig.scoreThresholds.minimum = Math.max(0, Math.min(100, Number(t.minimum)));
  }

  if (overrides.penalties) {
    const p = overrides.penalties;
    if (p.transpositionPerOctave !== undefined)
      ScoringConfig.penalties.transpositionPerOctave = Math.max(
        0,
  ...
  if (overrides.bonuses) {
    ...
    if (b.sameCategoryMatch !== undefined)
      ScoringConfig.bonuses.sameCategoryMatch = Math.max(
        0,
        Math.min(30, Number(b.sameCategoryMatch))
      );
    if (b.sameFamilyMatch !== undefined)
      ScoringConfig.bonuses.sameFamilyMatch = Math.max(0, Math.min(25, Number(b.sameFamilyMatch)));
    if (b.exactTypeMatch !== undefined)
      ScoringConfig.bonuses.exactTypeMatch = Math.max(0, Math.min(30, Number(b.exactTypeMatch)));
  }

  if (overrides.percussion) {
    const perc = overrides.percussion;
    if (perc.drumChannelDrumBonus !== undefined)
      ScoringConfig.percussion.drumChannelDrumBonus = Math.max(
        0,
        Math.min(30, Number(perc.drumChannelDrumBonus))
      );
    if (perc.drumChannelNonDrumPenalty !== undefined)
      ScoringConfig.percussion.drumChannelNonDrumPenalty = Math.max(
        -100,
        Math.min(0, Number(perc.drumChannelNonDrumPenalty))
      );
    if (perc.nonDrumChannelDrumPenalty !== undefined)
      ScoringConfig.percussion.nonDrumChannelDrumPenalty = Math.max(
        -100,
        Math.min(0, Number(perc.nonDrumChannelDrumPenalty))
      );
    if (perc.drumChannelWeights) {
      ...
      for (const k of ['noteRange', 'programMatch', 'instrumentType', 'polyphony', 'ccSupport']) {
        if (dw[k] !== undefined)
          ScoringConfig.percussion.drumChannelWeights[k] = Math.max(
            0,
            Math.min(100, Math.round(Number(dw[k])))
          );
      }
    }
  }
  ...
  if (overrides.splitting) {
    ...
    if (s.minQuality !== undefined)
      ScoringConfig.splitting.minQuality = Math.max(0, Math.min(100, Number(s.minQuality)));
    if (s.maxInstruments !== undefined)
      ScoringConfig.splitting.maxInstruments = Math.max(2, Math.min(8, Number(s.maxInstruments)));
    if (s.triggerBelowScore !== undefined)
      ScoringConfig.splitting.triggerBelowScore = Math.max(
        0,
        Math.min(100, Number(s.triggerBelowScore))
      );
  }

  if (overrides.routing) {
    const r = overrides.routing;
    if (r.allowInstrumentReuse !== undefined)
      ScoringConfig.routing.allowInstrumentReuse = !!r.allowInstrumentReuse;
    if (r.sharedInstrumentPenalty !== undefined)
      ScoringConfig.routing.sharedInstrumentPenalty = Math.max(
        0,
        Math.min(30, Math.round(Number(r.sharedInstrumentPenalty)))
      );
    if (r.autoSplitAvoidTransposition !== undefined)
      ScoringConfig.routing.autoSplitAvoidTransposition = !!r.autoSplitAvoidTransposition;
    if (r.preferSingleInstrument !== undefined)
      ScoringConfig.routing.preferSingleInstrument = !!r.preferSingleInstrument;
    if (r.preferSimilarGMType !== undefined)
      ScoringConfig.routing.preferSimilarGMType = !!r.preferSimilarGMType;
...
function restoreScoringConfig(original) {
  ...
  if (ScoringConfig.percussion.drumChannelWeights && original.percussion.drumChannelWeights) {
    Object.assign(
      ScoringConfig.percussion.drumChannelWeights,
      original.percussion.drumChannelWeights
    );
  }
...
async function generateAssignmentSuggestions(app, data) {
  ...
  return cache.runExclusive(async () => {
    ...
    if (cacheKey) {
      try {
        cache.set(data.fileId, cacheKey, response);
      } catch {
        /* non-fatal */
      }
    }
...
export function register(registry, app) {
  ...
  registry.register('generate_assignment_suggestions', (data) =>
    generateAssignmentSuggestions(app, data)
  );
}
