...
const CAPABILITIES_PATH = path.resolve(
  __dirname,
  '../../../shared/gm-instrument-capabilities.json'
);

/**
 * Canonical per-GM-program capabilities (note range + polyphony), loaded
 * once at module init. The frontend keeps an inline mirror
 * (public/js/features/GmInstrumentCapabilities.js); a Vitest test asserts
 * parity (tests/frontend/gm-instrument-capabilities-sync.test.js).
 */
const GM_CAPABILITIES = JSON.parse(fs.readFileSync(CAPABILITIES_PATH, 'utf8'));
...
/**
 * Default polyphony for GM instruments (0-127), derived from the canonical
 * capability table (shared/gm-instrument-capabilities.json). Represents the
 * typical polyphony of the real acoustic instrument. Used as a fallback when
 * the routed instrument has no defined value.
 */
const GM_DEFAULT_POLYPHONY = Object.fromEntries(
  Object.entries(GM_CAPABILITIES).map(([p, cap]) => [Number(p), cap.polyphony])
);
